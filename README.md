...
- **安装机制**：`setup.sh` 遍历仓库根下所有目录，仅安装含 `SKILL.md` 的（故 `docs/`、`hack/` 不会被当 skill）。共四个落点：
...
- **dogfooding**：本仓库自身的每一次变更都走这套流程（propose → 设计审 → 代码审 →
  `sdflow-done` 归档），规则先在自己身上跑通再发布。其 `openspec/` 工作目录属开发期资产，
  不随发布分发。
...
入口 `issues_v2.py` 详见 `sdflow-issues/SKILL.md`。零依赖 YAML 子集写读（**不引入 PyYAML**——
数据类 skill 一律零三方 Python 包，装完即用，不要求消费方先建虚拟环境）。
...
工作流的结构化全景（阶段 × skill × 脚本门 × 产物锚）见 [docs/workflow-map.md](./docs/workflow-map.md)；
叙事版详解见 [sdflow-guide.html](./sdflow-init/assets/workflow/sdflow-guide.html)。
