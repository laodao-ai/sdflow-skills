...
- **覆盖面** = `source.file`（非 `-` 时）+ `smoke` + lane **显式声明的** `fixtures[]`——**穷尽，无第四项**〔A22〕
  > **`fixtures[]` 的语义 MUST 钉死** = 本泳道验证所依赖、需纳入时效锚的**全部文件**：harness · testdata · **外部配置（`compose.yml` / `broker.conf` / lockfile）**。**MUST NOT 把「外部配置文件」列为与 `fixtures[]` 并列的第四项**——它**没有独立字段**（见 A22）。
