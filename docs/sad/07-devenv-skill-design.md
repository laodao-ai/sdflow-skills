...
#### ⭐ 两条问话纪律（**「怎么问」的约束** —— 操作者 2026-07-14 拍定，A30 接地）

**「靠问兑现」不等于「什么都拿去问」。** 人的注意力是本 skill 唯一消耗掉就补不回来的资源：
**每一个「你们前端用什么测试框架？」，都在挤掉一个「你上次被这个环境坑到是什么事？」**

| | |
|---|---|
| **P1 · 能查的自己查** | 答案在仓 / 在机器 / 在公开资料里 ⇒ **自己去拿，拿到直接给结论**。**MUST NOT 拿它占用人的注意力。** |
| **P2 · 不确定的先调研再给推荐** | 拿不准 ⇒ **调研 → 推荐 + 依据 + 代价 + 备选 → 人拍板**。**MUST NOT 甩开放题**（把三个选项原样丢给人 = 让人替你做调研） |
...
| 查得到候选与依据（选型 · 路线 · 工具） | **调研 → 带推荐进人门**（P2） |
| **只在人脑子里**（偏好 · 踩过的坑 · 拍板权 · 组织约束） | **问** —— **注意力该全花在这里** |

⇒ 三层框架的十五格里，**①②③④ 十二格在归位模式下大多「自己查得到」**（读 `go.mod` / `Makefile`／**直接跑一遍**）；
**⑥ 那三格查不到、只能问** —— 而它恰是整个 skill 的价值所在（§2.2）。**省下来的注意力全部投到 ⑥。**
...
**本文档零 JSON 载体、零机械渲染**〔D9 · **A27**〕——**但它有模版、有提问清单、由 skill 逐槽问出来。**

> **⚠️「有模版 + 逐槽问出口」和「有 JSON 载体」是两件事。** 早前把它们混成了一件，于是把本文档误判成「skill 只给个初稿」。
>
> **正解**：`references/environments-template.md` = **十槽提问清单**（与 `testing-framework.md` 对称）；`init` **铺十槽骨架**（全 `⚠️ 待定`）；§5 ① 步**逐槽问出来**；**铺完归人 own，skill 不再覆盖**。
> **不落 JSON 的理由**：这十槽是**长自由文本**（「新人最容易卡在哪一步」「有没有退不回去的东西」）。`06` 接地实测：17 槽里 SAD 真投影只有 2 个，**其余纯人写区恰恰是全篇最高价值的部分**。**把它强行 JSON 化，只会让人写得更烂。**

**⭐ 最贵的三槽：常见坑 · 回滚 · 构建副产物**——**它们没有标准答案，模型答不出来，只能问人，所以最容易被静默略过。**

> 「回滚」的灵魂是那一问：**「有没有退不回去的东西？」**（DB migration / 已发出的消息 / 已 OTA 到设备的固件）
> **大多数事故不是「发错了」，是「发错了退不回来」。**

**lint 只数它的 `⚠️ 待定`**（**一个固定字符串的计数，不切章节、不判结构**——语法面只有一个元素，穷举得完 ⇒ 合法〔基准 5〕）。**MUST NOT 演化成「找到 §1.5 这一节、切出内容、判断非空」——那是又一个手搓 Markdown 解析器**〔A20〕。
...
## 附录 C：首个真实试点的实证（A29–A30，2026-07-14）

> **试点对象**：`sarvelo/mqtt-console`（Go + Wails + Svelte，三运行时横跨，外部依赖 / UI / 语言桥三形态叠加）。
> **选它的理由**：它是本设计的接地样本——**三层框架如果在它身上都推不出来，那就是设计错了，不是执行问题。**

### 试点结论：**核心承诺兑现，A-8 拿到第一份实证**

**A-8**（「模型能为三层提出**像样的**验证方法」——这是整个设计的地基，此前**零实证**）：**成立。**

| | |
|---|---|
| **产出** | 13 条泳道横跨三层，**10 条真跑绿**（script fork，exit 0，带 commit 锚） |
| **不只是文档** | 真落了代码：`hack/doctor.sh` · `hack/e2e-live.sh` · `internal/store/schema_golden_test.go` + golden fixtures · `frontend/e2e-live/live.spec.js` + `playwright.live.config.js`；Makefile 加 `race` / `doctor` 两个 target |
| **⑥ 槽（最担心的一格）** | **产出的是真发现，不是套话** |

**⑥ 槽的实证**（它证明了 §2.2 的赌注是对的）：

> `frontend/e2e/happy-path.spec.js` 跑在手写的 `wailsStub.js` 上——那是一份对 Go 后端行为的**手写镜像**，
> **没有任何东西检查这面镜子照得对不对**（`assert-bindings` 只查 6 个函数名，而真实绑定面是 26 个）。桩注释里已出现 drift #3。

——而且它**把这个洞补上了**：新建 `wails-live-e2e` 泳道（真 Chromium → 真 Wails 绑定 → 真 Go → 真 mosquitto），跑绿。
同类还有两条：`schema-forward-compat`（发现 YAML 存储无 golden，顺带挖出阻塞发布的存储层缺陷 T49）、`race-detector`。

---

### A29：**层状态投影「一绿即绿」**（A25 的病换了个地方长出来）

| | |
|---|---|
| **原实现** | `layer_status`：`if "verified" in statuses: return "verified"` |
| **试点现场** | e2e 层三条泳道 —— `wails-live-e2e` 绿，`packaged-app-boot` / `packaged-app-visual` 都是 `planned`（**打包冒烟压根没做，而那正是「能不能交付」的唯一证据**）。标题照报 **「✅ 已验证」** |

**为什么这是 A25 本人**：A25 杀掉了「手写层状态」（它会撒谎），把状态改成从泳道**投影**。
**但投影规则本身取了最强的那条泳道** ⇒ 假绿只是从「手写字段」搬到了「投影函数」里。
**杀掉一个机制，不等于杀掉它的病。**

**更狠的一点**：这个假绿**曾被写进测试的断言里**——
`tests/test_schema.py` 原有一行 `(["planned","scaffolded","verified"], "verified")  # 多泳道：有一条绿就算绿`。
**即：它不是漏了，是被当成正确行为固化了。**

**修**：投影取**最弱**的那条泳道。全绿才 `verified`；有绿有非绿 ⇒ **`partial`**，标题如实写「**3 条泳道里只跑绿了 1 条**」。
**理由**：**标题那一行才是被读的那一行**——下面泳道表里那两个 `○` 救不了它。

---

### A30：**问了本可自查的东西 · 甩了没调研的开放题**（→ 正文 §0.1「两条问话纪律」）

试点跑完，`environments.md` 留下这么一格：

> **⚠️ 待定**：Windows 包到底怎么产出？（买/借一台 Windows 机器？上 GitHub Actions 的 windows runner？还是把 Windows 降级为 non-goal？）

**三个选项，零调研，零推荐。** 而「这仓有没有 CI」「wails 能不能在 windows runner 上原生打包」「公开仓 runner 免费额度」
**全是模型自己查得到的**。人接到的不是一个待拍板的决策，而是**一道作业题**。

**对照组**：紧邻的 Linux 那格它就做对了——「⚠️ 待定 —— **skill 建议定为 non-goal**」，给了推荐。
⇒ **不是「不会做」，是「没有纪律要求它每次都做」。**

**修**：正文 §0.1 立 **P1 / P2 + 三分判据**；`SKILL.md` 加「两条问话纪律」（与五条红线对称：红线是 MUST NOT，这两条是 MUST DO）；
`references/` 的十槽与十五格**逐槽标注归属**（自己查 / 调研后给推荐 / 只能问人）。

**顺带修的 F2**：模型在现场**自创**了一份出处图例（`[实测]` / `[代码]` / `[人拍]` / `[⚠️ 待定]`）——发明本身很好，
但它把 `⚠️ 待定` 这个**字面量**写进了图例，于是 `env_pending`（**数这个字面量出现几次**）**恒 +1**（报 3/10，真待定只有 2）。
**修法不是让 lint 变聪明去认图例**（那是又一个手搓 Markdown 解析器，基准 5），
而是**把这份好发明收进骨架、并让图例「指称而不复现」该标记** ——
「数一个字面量」成立的前提是**它只有一个意思**，而**维持这个前提是模版的责任，不是 lint 的责任**。
