...
>
> **叙事伴读**：想按「阶段故事」而非「字段速查」理解全流程 → [`sdflow-guide.html`](../sdflow-init/assets/workflow/sdflow-guide.html)（自包含单文件，随 workflow bundle 分发；装机后也在 `~/.sdflow/workflow/`）。本文侧重结构化映射（字段 × 脚本 × 判据），那份侧重流程叙事，互补不重叠。
...
| 5 | retro 复盘 | `sdflow-retro`（只读） | `retro_report.py` / `lens_metric_aggregate.py` | 不受 ship_gate 管 | `openspec/retro/report.md`（D12 待复评区块） | — |
...
| **log_check.py** | （计划未建） | 拟：TG-02 相关工具（宿主待定） | 拟：`--log serial.log --rules *.yaml` · 时间窗 · must_contain/not/before · severity rollup | **尚未建**（roadmap mechanical-layer-hardening 阶段4 · 子任务 4.A，按需排；survey 内部编号 P5，勿与已交付的「阶段5 gate→frontmatter」撞号） | 拟 bundle | roadmap:118 |
