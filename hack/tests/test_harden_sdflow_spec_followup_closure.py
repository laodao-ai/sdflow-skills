...
# 【发布 clone 边界】本仓 `openspec/` 是这些门的**被测语料**（台账 / 四件套 / 归档报告），
# 而发布用的 public clone 不含它 ⇒ 模块顶层的路径解析会 raise、pytest **collection 阶段
# 就 interrupt 整个 session**（2588 个用例一个都跑不成）。故：完整仓里照跑照红，缺
# `openspec/` 时**显式 skip 并说明**——MUST NOT 静默 pass（那才是假绿）。
_HAS_OPENSPEC = (ROOT / "openspec" / "changes").is_dir()
needs_full_repo = pytest.mark.skipif(
    not _HAS_OPENSPEC,
    reason="本门的被测语料是本仓 openspec/ 台账；发布 clone 不含 openspec/",
)
...
CHANGE = (_resolve_change_root(ROOT / "openspec/changes") if _HAS_OPENSPEC
          else ROOT / "openspec/changes" / CHANGE_NAME)  # 缺 openspec/ 时不解析，交给 skipif
