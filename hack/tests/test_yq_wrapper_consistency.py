...
plan frontmatter 的 YAML 值）——以下计数再去 1 份，改为 **5 份**。

5 份脚本各自内联一份 `_yq()`（design.md §1 决定：不跨脚本共享——各脚本「零依赖不变量」
不允许互相 import，且 `~10` 行的封装体量本就不值得为共享而抽公共模块）。「各自内联」
不等于「各自漂移」——本文件机械校验 5 份实现共享的**核心逻辑**骨架：
...
【已知且已在各自 docstring 记录的差异——不在本文件检查范围内】
  - `ship_gate.py` / `sad_schema.py` 额外支持 `text=` stdin 参数（frontmatter 消费方
    需要从 `git show` 结果/内存中的 text 读，而非只从磁盘路径读）；其余 3 份只接受
    文件路径。
  - `init.py` 额外带 `--header-preprocess=false`（`config.yaml` 的 `--- # 注释` 起始
    形态会触发 yq 吞行 bug，需要显式关闭该预处理；其余 4 份的消费文件不含这类写法）。
  - `init.py` / `ship_gate.py` / `roadmap_writeback_draft.py` / `sad_schema.py`
    有 F3 多文档防御（`json.JSONDecoder().raw_decode` + 检测 stdout 是否含一个以上
    JSON 值）；`anchor_lint.py`（Task 2 最早落地）用简单 `json.loads`，未加此防御——
    其唯一消费点查 `.metrics.enabled`（布尔叶子，由 sdflow-init 生成/管理的文件，
    非任意用户输入，F3 场景在此不适用）。
  - `sad_schema.py` 额外带 `object_pairs_hook` 重复键检测（其既有测试套件要求
    duplicate-key 场景 fail-closed；其余 4 份的消费点无此要求）。
...
本文件不做「5 份字节完全一致」的机械 diff——那会强迫无差异化需求的脚本背上不需要的
...
# 5 份 _yq() 消费点（design.md §2 改动清单 + Task 5 新增两份；fix-probe-scan-precision
...
def yq_sources():
    """{label: _yq() 源码文本}，5 份都取得到才算本 fixture 成功——取不到直接让依赖它的
    每条用例报错（比 skip 更诚实：golden test 的前提是"5 份都在"，任一份加载失败就是
    本身要抓的问题，不该被静默跳过）。"""
    out = {}
...
def test_all_five_targets_are_distinct_files():
    """5 个 label 各指向存在且互不相同的文件（防复制粘贴把两个 label 指向同一路径、
    悄悄漏掉真正该覆盖的第 5 份）。"""
    paths = list(TARGETS.values())
    assert len(paths) == 5, f"golden test 应覆盖 5 份 _yq()，实际登记 {len(paths)} 份"
    resolved = {p.resolve() for p in paths}
    assert len(resolved) == 5, f"存在重复路径：{TARGETS}"
...
def test_non_zero_exit_branch_fails_loud_not_silently(yq_sources):
    ...
    不得吞非零退出、也不得因 default 而静默）——这一分支 5 份统一用 `raise`（不像
