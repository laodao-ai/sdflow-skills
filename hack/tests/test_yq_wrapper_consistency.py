...
tools/anchor_lint.py` 条目保留。计数改为 6 份。

[remove-superpowers-pipeline Task 1] `sdflow-implement/scripts/impl_route.py` 的 `_yq()`
随管线路由半场整体切除退出消费面（tickets 成为唯一管线，路由三跳不再需要读 config/
plan frontmatter 的 YAML 值）。

[scope-aggregate-suite-by-diff-domain Task 1] `impl_route.py` 重新内联 `_yq()`——新增
`suite-scope` 子命令需要读 `test-suites.domains` 配置。计数变化 MUST NOT 在本文件写死
具体数字（见下方 `TARGETS`「份数不写死」注记）；`test_all_targets_are_distinct_files`
不再断言固定总数，只断言标签互不重复指向同一文件。

在册脚本各自内联一份 `_yq()`（design.md §1 决定：不跨脚本共享——各脚本「零依赖不变量」
不允许互相 import，且 `~10` 行的封装体量本就不值得为共享而抽公共模块）。「各自内联」
不等于「各自漂移」——本文件机械校验各实现共享的**核心逻辑**骨架：
...
【已知且已在各自 docstring 记录的差异——不在本文件检查范围内】
  - `ship_gate.py` / `sad_schema.py` 额外支持 `text=` stdin 参数（frontmatter 消费方
    需要从 `git show` 结果/内存中的 text 读，而非只从磁盘路径读）；其余在册目标只接受
    文件路径。
  - `init.py` 额外带 `--header-preprocess=false`（`config.yaml` 的 `--- # 注释` 起始
    形态会触发 yq 吞行 bug，需要显式关闭该预处理；其余在册目标的消费文件不含这类写法）。
  - `init.py` / `ship_gate.py` / `roadmap_writeback_draft.py` / `sad_schema.py` /
    `impl_route.py` 有 F3 多文档防御（`json.JSONDecoder().raw_decode` + 检测 stdout
    是否含一个以上 JSON 值）；`anchor_lint.py`（Task 2 最早落地）用简单 `json.loads`，
    未加此防御——其唯一消费点查 `.metrics.enabled`（布尔叶子，由 sdflow-init 生成/管理
    的文件，非任意用户输入，F3 场景在此不适用）。
  - `sad_schema.py` 额外带 `object_pairs_hook` 重复键检测（其既有测试套件要求
    duplicate-key 场景 fail-closed；其余在册目标的消费点无此要求）。
...
本文件不做「字节完全一致」的机械 diff——那会强迫无差异化需求的脚本背上不需要的
...
# `_yq()` 消费点（design.md §2 改动清单 + Task 5 新增两份；fix-probe-scan-precision
# task4 删除消费仓镜像条目——权威源条目保留，见上方 docstring 说明；remove-superpowers-pipeline
# Task 1 删 impl_route.py 条目——`_yq` 随路由半场整体切除退出消费面；
# scope-aggregate-suite-by-diff-domain Task 1 因 `suite-scope` 子命令需要而重新加回）。
# 份数不写死——以本字典实际枚举为准（本仓「别硬编码数字、让脚本自己报」取向，R12）。
TARGETS = {
...
def yq_sources():
    """{label: _yq() 源码文本}，在册目标都取得到才算本 fixture 成功——取不到直接让依赖它的
    每条用例报错（比 skip 更诚实：golden test 的前提是"在册目标都在"，任一份加载失败就是
    本身要抓的问题，不该被静默跳过）。"""
    out = {}
...
def test_all_targets_are_distinct_files():
    """各 label 指向存在且互不相同的文件（防复制粘贴把两个 label 指向同一路径、
    悄悄漏掉真正该覆盖的一份；不断言固定总数——份数不写死，见 TARGETS 注记）。"""
    paths = list(TARGETS.values())
    resolved = {p.resolve() for p in paths}
    assert len(resolved) == len(paths), f"存在重复路径：{TARGETS}"
...
def test_non_zero_exit_branch_fails_loud_not_silently(yq_sources):
    ...
    不得吞非零退出、也不得因 default 而静默）——这一分支在册目标统一用 `raise`（不像
