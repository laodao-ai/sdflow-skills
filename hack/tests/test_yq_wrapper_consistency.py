...
7 份脚本各自内联一份 `_yq()`（design.md §1 决定：不跨脚本共享——各脚本「零依赖不变量」
不允许互相 import，且 `~10` 行的封装体量本就不值得为共享而抽公共模块）。「各自内联」
不等于「各自漂移」——本文件机械校验 7 份实现共享的**核心逻辑**骨架：
...
【已知且已在各自 docstring 记录的差异——不在本文件检查范围内】
  - `ship_gate.py` / `sad_schema.py` 额外支持 `text=` stdin 参数（frontmatter 消费方
    需要从 `git show` 结果/内存中的 text 读，而非只从磁盘路径读）；其余 5 份只接受
    文件路径。
  - `init.py` 额外带 `--header-preprocess=false`（`config.yaml` 的 `--- # 注释` 起始
    形态会触发 yq 吞行 bug，需要显式关闭该预处理；其余 6 份的消费文件不含这类写法）。
  - `init.py` / `ship_gate.py` / `impl_route.py` / `roadmap_writeback_draft.py` /
    ...
    是否含一个以上 JSON 值）；两份 `anchor_lint.py`（Task 2 最早落地）用简单
  ...
  - `sad_schema.py` 额外带 `object_pairs_hook` 重复键检测（其既有测试套件要求
    duplicate-key 场景 fail-closed；其余 6 份的消费点无此要求）。
  - `ship_gate.py` / `init.py` 在 `front_matter=True` 且 `default is not None` 时对
    非 dict 顶层结构做 R5/F4 校验（design.md 参考实现原样保留该分支，即使部分文件的
    调用点当前不触发）；`anchor_lint.py` 两份副本同样保留该分支（design.md §1 原文）。

本文件不做「7 份字节完全一致」的机械 diff——那会强迫无差异化需求的脚本背上不需要的
...
# 7 份 _yq() 消费点（design.md §2 改动清单 + Task 5 新增两份）。
TARGETS = {
    ...
    "openspec/workflow/tools/anchor_lint.py":
        REPO / "openspec" / "workflow" / "tools" / "anchor_lint.py",
...
def _load_module(path, unique_name):
    """从文件路径加载模块，不注册进 `sys.modules`（避免同名模块——两份 `anchor_lint.py`——
    跨文件互相遮蔽，也避免污染其它测试文件的 import 状态）。"""
...
def yq_sources():
    """{label: _yq() 源码文本}，7 份都取得到才算本 fixture 成功——取不到直接让依赖它的
    每条用例报错（比 skip 更诚实：golden test 的前提是"7 份都在"，任一份加载失败就是
    本身要抓的问题，不该被静默跳过）。"""
    out = {}
...
def test_all_seven_targets_are_distinct_files():
    """7 个 label 各指向存在且互不相同的文件（防复制粘贴把两个 label 指向同一路径、
    悄悄漏掉真正该覆盖的第 7 份）。"""
    paths = list(TARGETS.values())
    assert len(paths) == 7, f"golden test 应覆盖 7 份 _yq()，实际登记 {len(paths)} 份"
    resolved = {p.resolve() for p in paths}
    assert len(resolved) == 7, f"存在重复路径：{TARGETS}"
...
def test_non_zero_exit_branch_fails_loud_not_silently(yq_sources):
    ...
    不得吞非零退出、也不得因 default 而静默）——这一分支 7 份统一用 `raise`（不像
