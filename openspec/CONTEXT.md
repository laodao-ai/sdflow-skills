...
- 「召回归机械、精确归语义 (Recall-Mechanical, Precision-Semantic)」〔`adr/0026`〕：门禁判定的机械层**只承诺召回**（任何实质改动都不漏），**不承诺精确**（可以误报）；误报由语义层（主 session 读 diff 判断）分诊，分诊结论 MUST 落盘留痕。实证：失鲜判定长期以来从 git 管道**推断**「被审内容变没变」，一轮 grill + 一轮多镜设计审在这条推断链上累计挖出**八个**缺陷（merge 帧恒空 / rename 吞源路径 / 非零退出折空串 / `-m` 重报锚前历史 / `--cc` 等于某 parent 时隐身 / 控制字符 C-quote / `diff.ignoreSubmodules` / `GIT_ICASE_PATHSPECS`），其中两对互为解药兼病灶、两个是外部可控态即可翻转判定——是同一根因的八张脸，非八个 bug。修法=录锚 + 直接比内容，让八者整类消失。**配套判据**：「监视集」（盯哪些路径）是**承重的**，「枚举」（从管道推断哪些路径被碰过）只是实现它的一种手段且是产地——**砍枚举、留监视集**；裸 `reviewed_sha == HEAD`（无监视集）已实测证伪：实现期勾复选框即让设计门永远失鲜。与基准 1 不冲突（机械层仍承担不可让渡的召回；让渡的精确率恰是缺陷产地），切分线仍按 [[mechanical-judgment-split-signal-criterion]]。
