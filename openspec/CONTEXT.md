...
- 「T10」曾被当作放之四海而皆准的单一决策协议引用——它实际只对"阶段三遇 ≥2 方案自动选"这一类场景成立(canonical 场景，`workflow.md`/`sdflow-ship`/`sdflow-code-review`/`spec-workflow` spec.md/`impl-orchestration` spec.md「粒度争议」「矛盾裁决」等落点共享)；`sdflow-implement` 借用同一处置形状(自动选/复核/defer)描述的"熔断仲裁"(同一发现连续 2 轮 re-review 仍未消解)，触发条件与前者本质不同，已独立成文、不再引用"T10"标签（见 `harden-implement-review-loop`/`adr/0031`）。
