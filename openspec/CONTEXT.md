...
- 「问题先于原语 (Question-before-Primitive)」〔`adr/0026`〕：门禁判据选底层原语（git 子命令、解析协议）时，MUST 先分别写出**每个消费方要回答的问题**，按问题选；**「两处共用一份实现」MUST NOT 作为目标被追求**——它是问题相同时的*结果*，不是可独立追求的价值。实证：失鲜判定的 design 域问「锚后每个碰四件套的提交是否都获豁免」（per-commit 判断 ⇒ 必须逐帧），code 域问「当前树与被审过的树在 `openspec/` 外有无差别」（状态比较 ⇒ 一次树 diff）；把前者的帧枚举推给后者，除了让它背一套不需要的循环，还连带继承 `git diff-tree -m` 的假阳。与「目标态论证」（`adr/0011`）互补：那条管**论证的基准**，本条管**手段的选取**；与基准 5「无界语法禁手搓」同源——都是「别用自己的实现去猜工具的语义，让工具自己回答」（`-m` 与 `--cc` 的差别不是详略，是问了不同的问题）。
