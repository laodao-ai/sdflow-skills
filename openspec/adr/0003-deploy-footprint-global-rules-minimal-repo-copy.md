...
>
> **进一步被取代（2026-08，`adr/0039`）**：本条所述「review 机械层脚本 → 仍复制进 `openspec/workflow/tools/`」与 resolver 的 local-pin 分支已由 `adr/0039`（消灭双链）取消——tools/ 与规则均改为全局单份共享，消费仓不再持有任何镜像，resolver 收缩为两步链（本地 pin 分支删除）。本文正文不改，指针见 `adr/0039`。
