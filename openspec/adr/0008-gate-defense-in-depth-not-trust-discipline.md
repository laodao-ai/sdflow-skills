# ship_gate 取防御纵深立场：不假设分支纪律成立，change 命名空间隔离即便有入口守卫也有价值
...
- **在 FF-0 里拦 stacking（禁 feature 分支上再建 change）**：从源头消除污染入口，gate 无需隔离。**未选为本 ADR 的解**：① FF-0 是通用跨项目守卫，stacking 有时是合法工作流（栈式变更）不该一刀切禁；② 把正确性寄托于"入口守卫无漏"违反防御纵深——守卫绕过（手工 `git`/其它入口）后 gate 仍会假✅。（FF-0 后来确实加了 halt-问人 + ack 逃生口，两条理由仍各自成立，本 ADR 的选择不变，见〔A-1〕。）
...
- Considered Options 里拒绝"在 FF-0 里拦 stacking"的理由 ①（"stacking 有时是合法工作流，不该一刀切禁"）由 **ack 逃生口**满足——新判据挡的是"静默默认发生"，不是"禁止 stacking"；
- 理由 ②（"把正确性寄托于入口守卫无漏违反防御纵深"）**完全不受影响**：守卫仍可绕过（人 ack / 取不到 change 名或 detached HEAD 时 fail-open / 手工 `git` 与其它入口）⇒ `ship_gate` 的 change-命名空间隔离 **MUST NOT** 因"现在 FF-0 会拦了"而被撤掉或降级。
