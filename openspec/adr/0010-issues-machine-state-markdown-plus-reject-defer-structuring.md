...
- **写时 reject + 延后结构化（选中）**：复用本仓既有 `_die` 写时校验惯例（priority/status 非法即拒），零解析器改动、fail-closed、对齐反静默纪律。实证支撑：全量扫过现存池**存量 0 行**字段含 `|`——"保留字面 `|`"**未来罕见但非零**〔spec-review C9 措辞降级：本仓 pipe 相关 bug summary 可能含 `|`，非"需求=0"〕，YAGNI 仍成立（错误响亮、可改写或用全角 ｜）。代价：真需要字面 `|` 的字段被拒（须改写），但响亮不静默。**守卫落点须在各命令入口原始参数、覆盖 add/triage/rename 全部管道表写路径**（非 `join` sink——spec-review C1 BLOCKER）。
