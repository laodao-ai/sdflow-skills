...
`ship_gate.py` 的 `parse_ship_gate_frontmatter(text)` 是 mlh-p5 定的**单一自持解析核心**（adr 家族①/A4 铁律「live 与归档 frontmatter 读共用同一严格核心」），被**三个**调用方共用〔spec-review BR-1/TC-1 订正：初稿误记为两个〕：`live_ship_gate_state`（L477，live 报告，走无锚语义分流）、`archived_verify_state`（L196，归档报告，D3 短路的 SHIPPED 判据源，absent 时**回退 inline dual-read**）、`anchor_set`（L412，熔断状态集 helper，`err≠None`→空集 / `state.items()`）。共用核心的好处是防三路解析漂移，**代价是**：改它任一返回语义，三个调用方**同时**受影响，且各侧的安全方向**可能相反**——live 侧 absent = 保守（不放行），归档侧 absent = 回退 inline（可能判 pass→SHIPPED），anchor_set 侧 absent 与坏都→空集（行为不变）。
...
- **mlh-p5-parser-cleanup 落地本 ADR**：design 订正「只影响 live」为「共用 helper、三调用方行为亦变、目标态两侧同向 fail-safe」；spec delta 补归档侧 Scenario（目标态漏闭合 frontmatter + 无 inline → `none` 不 SHIPPED）；tasks 补该目标态归档回归测试；`ship_gate.py`「已知不覆盖」登记「`---` 打头 × inline PASS 杂交（无 producer 产出，须手工越权构造）」盲区。
- **本 ADR 首次落地自证纪律必要性〔spec-review BR-1〕**：初稿 design/ADR 只数了 live+归档两个调用方，**漏了第三个 `anchor_set`**——三镜（codex + Claude eng subagent + 测试完备镜）独立命中。恰印证「改共用核心 MUST 对每个调用方分别论证」的必要（连本 ADR 自己起草时都漏），故此纪律 MUST 配一步机械核验：改 `parse_ship_gate_frontmatter` 前 `grep -n 'parse_ship_gate_frontmatter' ship_gate.py` 列全部调用点作论证清单，MUST NOT 凭记忆枚举。`anchor_set` 侧论证：`err≠None`(旧 unterminated) 与 `state={}`(新 absent) 两路都 `→ frozenset()` 空集，熔断进展判据无净变化倾向，**行为不变、安全**——但 MUST 有 pytest（`test_anchor_set_absent_on_unclosed_frontmatter`）钉死此不变量，防未来重构 anchor_set 短路时无声失守。
