...
**DD5 retro 标签解析与墙钟归属**。历史 `checkpoint(spec-review-autoplan)` 标签的**识别**由既有前缀规则天然覆盖(零代码改动,`test_retro_report.py:129` 现绿),SKILL 侧不再产生。**墙钟归属〔spec-review-amendment Q1 推荐,设计门确认〕**:`stage_walltimes` 相邻提交差由「计入前一提交阶段」改为「计入后一提交阶段」(checkpoint=工作完成点)——修正既有错账(现口径 Step1 广审墙钟经 `("sdflow-spec-generate","ff")` 误归 ff)并使单 checkpoint 下 spec-review 归属正确,49 归档 change 按新口径统一重算再生(见 tasks 1.3)。**备选均被核验证伪/砍掉**:「解析器删除该前缀」→ 历史归类漂移;「镜收齐后落新中间 checkpoint」→ attribute-to-previous 语义下 generate→dispatch 区间仍误归 ff,修不了账;「评审起点空 checkpoint」→ checkpoint-commit.sh 干净树跳过、无 --allow-empty。中间 checkpoint 不保留,崩溃恢复点损失按五问接受(概率低/影响=重跑镜/镜产出在会话 transcript 可捞)。
