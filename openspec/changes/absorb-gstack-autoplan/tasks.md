...
- [ ] 5.1 review 节重写:判定点②(商业化分档)删除、判定留痕总则三判定点改两判定点、恒跑 strategy/plan-eng 双镜(镜职责经 2.5 的同源注入托管块承载,MUST NOT 无守卫手工拷贝)、失败处置改述(未审待恢复语义不变)[spec-review-amendment Q2 措辞同步]〔Req: roadmap-planning·review 恒跑自持双镜与跨模型声〕
...
- [ ] 6.5 归档盲测(逐声边际贡献)〔Q3 推荐,设计门确认后执行;门:任务组 2 完成后(需新镜 prompt)、发布前〕:选 3-5 份含 broad 独家 findings 的归档 change,同一语料**分别单独**跑 strategy 镜 / plan-eng 镜 / design-voice 三声,测①旧 broad 独家高危 findings 召回率 ②各声**边际独家召回**(为「双镜是否都值得每轮付费」铺证据路径——F-adv1:聚合 broad 独立率 33% 全镜种最低,C2 n=2 均出自 Codex 腿);盲测报告随本 change 归档,作 D1 形态的留存/降声证据基线〔proposal·假设〕
