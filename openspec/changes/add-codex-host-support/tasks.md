...
- [ ] 0.2 **A3 核验**：在**真实 Codex 宿主 session** 内冒烟 `claude -p --model opus --output-format text --tools "" --strict-mcp-config`〔spec-review-r2 C4：零工具〕——核验点是**"能否成功发出网络请求并拿到 findings"**（非"claude 装没装"）。Codex 若封出境网络 ⇒ 反向 voice 恒失效、永远同族 fallback，design 须承认该 scope 边界〔host-adaptive-execution · outside voice = 另一个机队的强档〕
...
- [ ] 7.3 TDD：**安全回归锁（承重墙，spec-review-r2 C4 零工具）**——加测试断言反向路径（claude）走同一个 `secret_scan` 与同一个 `render_prompt`；secret 命中时**两条路径都 exit 3 拒发且不 fallback**、**且 `secret_scan` 的 stderr 只出规则类型+行号、不出命中原行/匹配值（D8），断言测试密钥不现于 stdout/stderr/临时日志**；**且断言 claude exec 行含 `--tools ""` + `--strict-mcp-config`、不含任何 repo-read 工具（`--tools "Read…"`）、不含 `--disallowedTools`、不含 `--allowedTools`**（防漂移回给工具/denylist/弱 allowlist，C4）〔host-adaptive-execution · secret 命中时两条路径都拒发（且不泄进日志）/ 只读约束按 runner 落到对应机制〕
- [ ] 7.4 实现反向 runner 调用：`claude -p --model "$SDFLOW_VOICE_MODEL" --output-format text --tools "" --strict-mcp-config`〔spec-review-r2 C4：**零工具**（`--tools ""`，把全部内建工具移出可用集）——首轮给 `Read/Grep/Glob` + `--add-dir` 的缓解「网络禁则无法外传」逻辑不成立（`claude -p` 本身联网、Read 内容随请求出境）；零工具只吃已扫 context、对称 codex、出境面 100% 被 secret_scan 覆盖。**只改跨模型 `claude -p` 反向路径，不改同族 fallback 子代理**（后者要读本仓找漏，与主审同信任级）〕〔host-adaptive-execution · 只读约束按 runner 落到对应机制〕
