...
- **路径 containment 校验**：`source.file` / `smoke` / `fixtures[]`（**含外部配置文件——它没有独立字段**）/ touched-files 清单全是模型填的自由文本，**MUST 经统一 containment helper**（拒绝绝对路径 / `..` / symlink 祖先 / 仓外 realpath）后才能读/写/删。
