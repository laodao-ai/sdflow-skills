...
报告 SHALL **原样透传** `devenv_lint` 的诚实后缀（`structure-ok-SEMANTICS-UNCHECKED`），**MUST NOT** 二次简化渲染成「verified = ✓」式的绿色状态——**结构通过 ≠ 内容已审**〔codex round-3〕。
...
**诚实边界（MUST 显式登记，MUST NOT 佯装）**：`sdflow-maintain` 是**人主动跑**的 ⇒ 本条提供的是「**更响的提醒**」而非**硬门禁**（无 `ship_gate` 式硬拦截）。是否再加硬拦截 → `add-sdflow-devenv` proposal 的 Q-6。
