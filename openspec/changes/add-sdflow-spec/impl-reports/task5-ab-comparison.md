...
| **7.2 subagent 路成本与质量均不劣于 thin 路** | ❌ **不达标** —— 成本**劣于**（+29% 美元 / +43% token），质量**不优于**（冷审 Important 1 vs 0） | 本报告 §3 |
| **7.1 机械层全量** | ✅ 全绿 | §8 |
...
| # | 验收标准 | 状态 | 证据 |
|---|---|---|---|
| 1 | 三路各跑一次同一真实复杂 change，五项指标齐全且可复核（不是估算） | ✅ 四项实测 / 1 项口径降级 | §2 实验设计 · §3 指标表；每一次调用的完整 JSON（含 `usage` / `modelUsage` / `total_cost_usd` / `duration_ms`）逐轮落盘，见 §2.4 |
...
- 全部 JSON 落 `scratchpad/ab/logs/<lane>/turn*.json`（**沙箱产物，不带回主仓**；数据已誊进本报告）。
...
`*` **已知污染（如实标注）**：thin/legacy 的 turn02 被我的驱动脚本外层 10 分钟上限 SIGKILL 掉
（`err=True`、`duration_ms=598753`），subagent 未被打断。会话状态持久化、下一轮 `-r` 续跑，
工作未丢失（thin turn03 直接交出完整四件套），但**多了一次 turn 边界的 prompt 处理**。
⇒ **该污染只会抬高 thin/legacy，不会抬高 subagent**；结论「subagent 贵于 thin」因此**方向稳健**：
要让 subagent 反超，thin 的真实成本得比实测再高 29%，而污染是反方向的。
...
| 7.2 成本 | subagent **$11.68** vs thin **$9.06**（**+28.9%**）；token 12.57M vs 8.81M（**+42.7%**）；墙钟 −7.6% | **劣于** ❌ |
...
1. **成本方向明确且抗污染**。唯一的实验污染（thin/legacy 各被外部中断一次）是**抬高 thin**的方向；
   subagent 要反超需要 thin 的真实成本再高 29%。∴ 方向结论稳健，**幅度**不稳健。
2. **省下来的和多花的不在一个量级**。外派确实把 opus context 压了 18.9%（省 $1.04），但
   researcher(haiku 3.55M) + writer(sonnet 1.88M) 自己烧掉 $3.66。这不是调参能补的差
   ——子代理的 context 不是免费的，而 `design.md` NFR 的估算方法**只算了档位差、没算这一项**（§3.5）。
3. **质量三项无一占优**，且论证密度上有一条**可指名的承重反例丢失**（§5.1）。
   即便成本打平，「不劣于」也不成立。
...
4. **N=1，非统计显著。** 一个 change、一次跑、一台机器、一个模型机队、一天之内。
   ...
   能说的是：**在这一次观测里方向是这样，且该方向抗已知污染。**
5. **实验污染已知且未消除**：thin/legacy 各有一轮被驱动脚本的 10 分钟上限中断（§3.2）。
   已论证其方向只会抬高 thin/legacy，但**幅度**因此不可信。
...
10. **回退动作的完整性**：我只改了 `sdflow-spec/SKILL.md` 两处。
