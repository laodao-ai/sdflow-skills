...
**强制阅读清单 SHALL 显式写死，MUST NOT 表述为「依赖产物」** [spec-review-amendment]：实跑 `openspec instructions design/specs --json` 核验，二者 `dependencies` **都是 `[proposal]`** ⇒ CLI 报告的真实图是 `proposal → {design, specs} → tasks`，design 与 specs **互不依赖**。若照字面按 CLI 依赖走，specs 生成步不会读 design.md。清单 SHALL 为：design 读 proposal；specs 读 proposal **+ design**；tasks 读 proposal + design + specs。
