...
- [ ] SA-11 的七处逐处已同步，无一遗漏；托管区块经刷新机制生成（未手改块内）
- [ ] G1 例外在其**两处载体**上各自成立（二者字面措辞不同，须分别核验，MUST NOT 指望一条 grep 同时命中）
- [ ] 机械核验的 grep 锚**打得中真实结构**：跨行 ASCII 图不得用单行正则当判据（该模式实测零命中 = 永不变红的空判据）；无可靠单行锚点者如实标为人核
- [ ] 生成物已随源重生成，其阶段一段落反映新分支
- [ ] FF-0 三分支判定在规则文本与其 hook 实现上一致（hook 原先只挡保护分支）
- [ ] 四入口选择规则在人读侧与 AI 读侧各有一份，内容不矛盾
- [ ] 旧入口 sunset 条件已写死阈值与未达标处置
- [ ] README 的 skills 列表含 `sdflow-spec` 且有可复制 Quick Start；重跑 `setup.sh` 后双 runtime 可见
...
- [ ] dogfood 演练在一个非玩具需求上跑完 A→B→C，八项核验逐条有证据（不是「看着过」）
- [ ] 六种故障各注入一次，处置与失败模式表一致：脏树/错分支 halt 问人、分支已存在走 fallback、陈旧纪要拒绝进 C 并呈摘要、CLI 缺失与 schema 不符 fail-closed 且报实际版本 + 修复命令
- [ ] `/clear` 后冷读产物，砍掉的候选与理由可追溯；报告标注「N=1 自评，非统计显著」
- [ ] retro 的阶段一归因率相对基线有改善，或如实记录未改善及原因
- [ ] 两项未核/后续工作已登记进 todolist，**显式带 `change` 字段**（省略会误挂当前活跃 change）
- [ ] 阶段一验收门结论明确落纸：本票 + Task 1 + Task 2 全过 ⇒ 方可启动阶段二
...
- [ ] 三路各跑一次同一真实复杂 change，五项指标齐全且可复核（不是估算）
- [ ] 论证密度比对有具体举例（哪条砍掉的候选在纪要驱动路径上丢了/留了），不是形容词级结论
- [ ] 阶段二验收门结论落纸：安全前提是否满足、成本质量是否不劣于 thin 路、判 GO 还是回退
- [ ] 判回退时：回退动作已执行到位，agent 定义的去留有明确处置，hand-off 如实记录
- [ ] 成本结论标注样本量与统计显著性边界（N=1 不得表述为已证实）
