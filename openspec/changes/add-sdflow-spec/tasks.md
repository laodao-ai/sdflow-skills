...
- [ ] 1.1 编写 `sdflow-spec/agents/sdflow-researcher.md`：frontmatter（`model: inherit`、`effort: low`、`tools: Read, Glob, Grep, Bash`）+ 正文角色纪律（只读供证、结论+file:line、材料不回传）+ 通则托管块占位 〔SA-07, SA-02〕
- [ ] 1.2 编写 `sdflow-spec/agents/sdflow-spec-writer.md`：frontmatter（`model: inherit`、`effort: medium`、`tools: Read, Glob, Grep, Bash, Write`）+ 正文（纪要压缩 / 单产物生成、自调 `openspec instructions`、读依赖产物、禁 AskUserQuestion）+ 通则托管块占位 〔SA-07, SA-05〕
...
- [ ] 2.3 相位 B 拷问指令：锚点纪要压缩派发、承重约束优先攻击、停止信号（共识+约束站稳）、决策纪要字段与亲笔要求、ADR/术语惰性提议钩子（三条件判据 + 最小模板）〔SA-03, SA-04, SA-10〕
- [ ] 2.4 相位 C 生成指令：FF-0 → `openspec new` → status 依赖序串行派 spec-writer（纪要下发 + 自取 instructions + 自读依赖）→ 写后核验 → 终审（纪要↔产物一致性，判断改/措辞放过）→ 纪要并入 design.md 决策记录 〔SA-05, SA-06, SA-04〕
...
- [ ] 3.1 CLAUDE.md/AGENTS.md 阶段一规范：新增 `sdflow-spec` 使用路径与出口序列约定；修正「grill-with-docs 来自 superpowers 插件」→ Matt Pocock skills 集合（`~/.agents/skills`）；标注三个原 skill 保留与适用场景 〔SA-09〕
