...
- [x] 版本门覆盖 `<1.7.0`、`1.10.0`、命令缺失和非数字输出
- [x] 迁移覆盖缺绑定补写、已有绑定 no-op、archive/stray 跳过、单项失败阻止 config 切换
- [x] copy bundle 覆盖权威源删除文件后的孤儿清理
- [x] schema 内容覆盖 id/generates、委派标记和两条 requires 边
- [x] update 模式 schema 单键改写覆盖且其余 config 内容 byte-identical
- [x] 每条新增测试均先通过定点破坏验证非恒真
- [x] 全仓 `pytest`：用户明确批准在 90 秒超时后跳过；实际结果为超时退出码 124，未宣称通过
...
- [x] 单元测试证据齐全并通过（变更相关聚合通过；全仓 pytest 按用户批准跳过并记录超时 124）
- [x] 集成测试证据齐全并通过，或记录未覆盖及判定依据
- [x] e2e 测试证据齐全并通过，或记录未覆盖及判定依据（当前无可发现自动化 e2e 套件，已记录未覆盖原因）
