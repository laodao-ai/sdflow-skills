...
三份 recorder（`buglist.py`/`todolist.py`/`issues.py`）各自内联持有的共享 helper MUST 由**剥函数 docstring 后 `ast.dump` 相等**的断言守护其**行为**不漂移，按实测拓扑分组：3 向组（`atomic_write`/`repo_root`/`_reject_cell_unsafe`）三份 AST 等价、2 向组（`detect_change`/`normalize_doc_paths`/`auto_default_doc`/`split_sections`/`parse_table_rows`/`block_ranges`/`_ids_in_files`/`_find_row_file`）buglist↔todolist AST 等价。契约锁**行为等价层非字面层**（grill 实测证伪「逐字同款」前提）：docstring/注释差异合法（按 recorder 语境分化，如 issues 记录内联原因），**不**算漂移；逻辑分叉（AST 不等）才报红。测试 MUST NOT 抽公共模块或跨 recorder import（守 D4 隔离）。
...
- **WHEN** 八个表解析/文档 helper 任一在 buglist↔todolist 剥 docstring 后 AST 不等
- **THEN** 一致性测试断言失败；且测试范围 MUST NOT 把 issues.py 纳入这八个 helper 的比对（issues 依 D4 不含表解析 helper）
...
#### Scenario: 现状基线（含 2 个逻辑异写归一后）
- **WHEN** 对当前三份 recorder（本 change 已把 `split_sections`/`block_ranges` 的 todolist 侧归一到 buglist 写法——block_ranges 两处差异均归一）跑一致性测试
- **THEN** 全部断言通过（剥 docstring 后全 11 个 helper AST 等价）
