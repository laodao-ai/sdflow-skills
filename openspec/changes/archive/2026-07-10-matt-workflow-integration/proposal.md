...
实现段是全流程第二大成本块且近纯 agent 时间：retro 聚合①实证 impl 阶段 1306.6 min、占 31%（30 change / 70.1 hr），而阶段三过设计门后无人类门。打实现段而非占比更大的 spec-review（39%）另有实证：wco roadmap P0/P2 已实测证伪「压缩 spec-review 内 agent 计算可动墙钟」（其耗时由人读/拍板 elapsed 主导，见 openspec/roadmaps/workflow-cost-optimization/roadmap.md）〔spec-review-amendment C3〕。现管线 writing-plans → subagent-dev 的成本是结构性「不信任税」（为弱执行模型校准）：计划期预写全量代码（写两遍）、2-5 分钟步长切 8-15 task（派发循环 ≥2N）、重型交接补丁（Interfaces/task-brief/ledger），superpowers 自记翻车账单（42k 字符 dispatch、final-review 修复波贵过全部任务）。2026-07-10 探索会 + 三镜/对抗镜设计评审已裁决混血方案（调研与六问裁决全文：docs/workflow-skills/impl-pipeline-matt-vs-superpowers.md §1-§10），估实现段 token 砍 40-60%，且 ship_gate 零改动兼容（完成判据契约 = 文件名 + `### Task <n>:` 标题 + checkpoint∪复选框，ticket 体不设限，§5 逐行亲验）。
...
| 试点样本有效性〔spec-review-amendment F3a〕 | 每试点 change 的 PIPELINE_RECEIPT 与 marker/config 意图一致 → 不一致者剔除样本 | receipt 留档，计入判赢集前逐 change 核对 |
...
- **增**：`sdflow-implement/SKILL.md`（新编排 skill）+ `sdflow-implement/scripts/`（stdlib-only route/拓扑 helper + pytest——spec-review-amendment F4/F8，原「无 scripts/」定性翻转）+ `sdflow-ship/tests/` golden-file 回归〔F6〕。
...
- **技术栈触发**：TG-01/02/03 不命中（纯 Markdown 编排 + 配置注释，无领域清单选用）；TG-18 **命中〔spec-review-amendment F6 翻转〕**：route/拓扑 helper pytest + golden-file 回归测试（producer→parser 契约，先例 test_producer_parser_contract）；gate 自身零改动、其既有测试不动。
