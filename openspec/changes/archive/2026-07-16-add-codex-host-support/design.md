...
- **正解（对称恢复）**：反向 claude 用 `--tools "Read,Grep,Glob"`（只读工具集，**无 Write/Bash/WebFetch**——无写、无独立网络工具，round-2 的致命外传向量仍被拒）**+ `--strict-mcp-config`**（隔离 ambient MCP）**+ `--add-dir <repo_root>`（实测 claude CLI 真有此旗；语义是**增量授权**"加可读目录"、非"收窄边界"，但 repo_root 由 cwd 反求、cwd⊆repo_root 故只补齐到仓库不引入仓外——r3-narrow 已证）**——与 codex `-C repo_root -s read-only` **对称**：都能读仓库找漏、都无写/无独立网络/无 MCP、读内容都随请求出境到各自模型商（FRAME 缓解）。**注**：claude `Read` 与 codex read-only 均**无硬 FS 读边界**（都可读仓外 `~/.ssh`/`.env`），此面同构、均靠 FRAME 缓解。
...
| # | 面 | 文件 | 改什么 |
|---|---|---|---|
| 1 | 契约单一源 | `assets/workflow/lens-metric-contract.md` | `lens-metric-enums` 块（`runner: claude,codex,none,unknown` + 新增 `host` + **`reason_code` 8 值域**：`ok`+4 降级+3 无执行）· `lens-metric-fold` 块（删 `claude-fallback:` 行）· 锚形 · 散文注记 · **〔r3-narrow #3 修正〕合法组合矩阵的关系式判定逻辑 NOT 落可 parse 机读块**（平铺 enums 块装不下 `runner≠host` 等关系式）——契约块只承载**枚举域**，矩阵逻辑由 anchor_lint + outside_voice_guard 各自本地重实现、全笛卡尔 golden 守（scope-check 未来改 reason_code/矩阵须同步：契约枚举域 + 两工具矩阵逻辑 + golden + workflow-map 字段表） |
...
| 风险 | 缓解 |
|---|---|
| **🔴 efficacy 押在未验假设 A1/A3（Q3/C5）** | **本 change 最重的风险**：headless/CI（Codex 主力形态）下若 `CODEX_THREAD_ID` 缺（A1）⇒ 永远 fail-loud 不跑 voice；若 Codex 封网络（A3）⇒ 永远同族 fallback（贴诚实标签的自审）⇒ **efficacy=0**。失效方向"安全"（不假绿）但**目标未达成**。缓解：**A1/A3 真机核验作前置门（Migration step 0），改契约前先证**。**〔spec-review-r2 C5：efficacy 前置门诚实归人门纪律，MUST NOT 造假机械锁〕**——「A1/A3 真的在**真** Codex 宿主验过」这件事的捕获环节由主 session（被监管方）把持，无可信捕获路径（marker 里塞个真格式 uuid 也是自报，同探针/§0.0 之坑）。∴ 前置门**由设计 HARD-GATE（人核对"验了没"）+ 冷审复核守**，不硬造 `.efficacy-gate-passed` 之类假机械门；proposal MUST 显著登记「若走过场则 BREAKING 建在未验假设上、Codex 主力形态可能零交付」。 ／ **实测进度（2026-07-16，真 Codex 宿主）：A1 交互形态 ✅、A3 交互形态出境 ✅**（`HOST=codex` + `outside-voice.sh exec` exit=0 反向 `claude -p` 返回真 findings）——交互形态 efficacy 非零、已坐实。**剩 headless/CI 形态未验**（那里若无 claude CLI/认证则 A3=0）；证据与判读见 `codex-verification-checklist.md`。〔勘误 2026-07-17：交互 smoke 的「efficacy 非零已坐实」被真实评审负载证伪——mlh-p6 spec-review 实跑 A3 efficacy=0（3/3 outside-voice 300s timeout、降级同族）；根因非 headless 缺 CLI，而是同步 `claude -p` 推理时长 >300s。机制改动方向=exec 改异步（规划中）。详见本目录 `codex-e2e-efficacy-report.md`〕 |
