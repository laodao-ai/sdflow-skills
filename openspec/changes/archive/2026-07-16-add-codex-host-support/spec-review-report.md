...
| **D6** | `runner="none"` ∧ `findings=0` 无机械交叉校验（各字段独立校验、不联动）；`runner="none" findings="5"` 通过全部检查，削弱「grep 可机械筛降级轮」可观测性承诺 | 对抗镜B F5 · hr-tg #1（2 源） | 并入 Q1 状态矩阵：`runner=="none" ⇒ findings=="0"` 校验。**〔add-codex-host-support code-review B1 订正——原「lens-metric 锚 + outside-voice 锚均查」措辞与实现有出入，据实澄清〕**：实现分两个校验器各司其职——**`sdflow:outside-voice` 锚**由 `check_legal_combo` 走合法组合矩阵（`runner="none"⇒findings=0` 属矩阵 no-exec 子句）；**`lens="outside-voice"` 的 lens-metric 行**由 `check_lens_metric` 的 OV 分支查结构不变量（B1 补：`runner="none"⇒findings=0`、`host="unknown"⇒runner="none"`、runner∈{claude,codex,none}）。二者**均查该不变量、但作用于不同锚种**，非同一校验器查两遍。钉进 F6/F7 + B1 的 5 测试。 |
