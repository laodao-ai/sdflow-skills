...
**🔴 D2.2 — R1〔code-review-fix1 · 登记不修〕高频×多类型混合信号风暴可整体击穿 trap 机制，性质与 (a)(b)(c) 不同**：M5（ov_cleanup 入口屏蔽 INT/TERM/HUP + 原子快照 PID）与 M6（trap 安装合并为一次调用）治好的是「**慢速/单一类型**」信号下的窄时序缝——它们不解决、也从未打算解决下面这条。

| # | 残余 | 触发条件 | 实测复现率 | 与 (a)(b)(c) 的性质差异 |
|---|---|---|---|---|
| **(d\*)** | **混合高频信号风暴可使 trap 机制整体失效**（不是某个窄时序缝，是 trap 本身没跑） | 3 秒内以 20–150ms 随机间隔【交替】发送 SIGTERM/SIGINT/SIGHUP | **15 次跑 10 次（67%）** | (a)(b)(c) 是【单指令级窗口】，概率极低、只影响那一次信号；(d\*) 是**整条 trap 机制**在信号交付层面被压垮——helper 进程被信号**默认处置**直接终止（非自身 `exit`），stderr **完全没有** `ov_cleanup` 痕迹，runner 与孙进程**双双存活**成孤儿 |

**实测细节（对抗镜 A，已由 M5/M6 落地后的当前实现复验，非修复前旧版单独实测）**：
- **对照组 1（单一信号类型同频洪泛）**：同样 3 秒 / 20–150ms 间隔，但只发一种信号（如连续 TERM）——**0/10 复现**，trap 正常跑完、runner 与孙进程正常回收。
- **对照组 2（慢速多类型信号）**：多类型交替但间隔放宽（非高频）——trap 会被重入触发多次，但 M5 的原子快照 + 幂等设计扛住，**不产生孤儿**。
- ⇒ **引爆点是「高频 × 多类型」的交集**：不是任一单一维度（速度或类型）单独造成，两者叠加时 bash 对连续到达的【不同】信号的排队/合并处理会在某个比例下整体丢弃后续 trap 调用（观测到的现象是 helper 进程本身被未处理的信号终止，而非 trap 执行后正常退出）。
- **回归见** `test_mixed_high_frequency_signal_storm_can_defeat_trap_mechanism`（`test_outside_voice_child_lifecycle.py`）——锁的是「该现象在当前实现下仍可复现」，不是「这个行为是对的」。

**为什么本轮 MUST NOT 修**：对抗镜给出的修法方向（外层去抖节流 / 用 `flock` 单实例互斥取代 bash trap 作为回收原语）是**换掉整个回收机制**，属**设计级决策**（需要评估：去抖会不会吞掉合法的连续信号？`flock` 互斥会不会引入新的死锁/饿死面？两者都超出「代码审」能拍板的范围）。本 change 的授权范围是修代码审揪出的具体缺陷，不是重新设计子进程回收架构——∴ 这里**只诚实登记**，修法留给未来一个专门的设计级 change 决策。

**MUST NOT**：声称本残余已通过 M4/M5/M6 一并解决；弱化其严重度描述为"极少见"或"可忽略"（67% 复现率不允许这么形容）；把它和 (a)(b)(c) 混为一谈（后者是无解的窄窗口，前者理论上有解，只是解法是设计级的）。
