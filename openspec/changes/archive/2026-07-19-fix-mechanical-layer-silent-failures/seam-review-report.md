...
接缝复审判定**收敛困难**后，经拍板**拆分**：

- **本 change 收缩为 B9 + B10**（`outside-voice.sh` 的截断字符边界安全 + runner 子进程回收）——两条均有实测支撑，三轮评审在其上零方向性错误。
- **S3 / S4 / S5 / S6 / S7 / S10 / S11 及 S1/S2 的 recorder 部分** → 随 **B11/B12 + recorder 三份物理复制 parser 的结构性根因** 合并为 **`T170` 统一处理**。**MUST NOT 只捡 B11/B12 单独修**——那正是本轮证否的路径。
- **S8 / S9（sidecar 身份与生命周期）** → 随 R7 拆出为 **`T171`**，独立成题时须先把六类「无 render」合法态的完整状态矩阵实跑验证。
- **voice 提的 stale-phrase 门**（机械禁止已被推翻的措辞出现在非考古区）→ 归入 `T170` 的调研范围，它是「返修不传导」的机械化解法。

**本报告的 S1/S2 中属于 B9/B10 的部分**：无——两条 critical 全部落在 recorder 侧的退出码与函数复用口径上，随拆分一并移出。
