...
**编排层自主决策下 MUST NOT 打 `task6-` 完成标签** —— 打了就是假绿（gate 的标签通道会把本票算完成）。
（**后续经真人授权改判，见文末「人拍板」节**——标签通道被显式启用，复选框仍保持未勾。）
...
## ✅ 人拍板（2026-07-26）：选项 A —— 降级合并，缺口移交后续 change

真人用户在被完整告知「Success Metric #1 未证」后明确选 **A**（三选项 A 现在合并 / B 等 07-29 补跑 /
C 挂起）。按四条通则开头的豁免条款——**真人明确指示优先，范围是人的决定**——编排层照办。

**降级的准确边界（合并后仍然成立的事实，任何人不得据本节反推为「已证」）**：

| 项 | 状态 |
|---|---|
| efficacy 三门（G1/G2/G3） | **未跑**，无 evidence JSON |
| `T162` | **保留 OPEN**，不关闭 |
| `design.md` / `CONTEXT` / hand-off 的「Codex efficacy=0」陈述 | **一字未改** |
| `superpowers-plan.md` Task 6 验收框 1 / 2 / 3 | **保持未勾**（永久诚实记录，MUST NOT 事后补勾） |
| 补证承接 | **T225**（新记，含额度恢复时间、复用的两条既有真实证据、验收路径） |

**完成信号只走 checkpoint 标签通道**，slug 与 commit message 直书「人授权降级、efficacy 未证」——
即：gate 进 6/6 这件事本身在 git 历史里是**带理由的显式动作**，不是一次沉默的补勾。
选择标签通道而非补勾复选框，正是为了让 plan 文件里那三个空框**永久留白**。
