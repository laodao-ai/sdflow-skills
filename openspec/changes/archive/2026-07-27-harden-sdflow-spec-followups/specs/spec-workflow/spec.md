...
- **THEN** [spec-review-amendment] FF-0 SHALL 不以 payload 仓的分支作 deny/allow 判定，并向上下文记录 `command-unverifiable`

#### Scenario: 动态 change 名
- **WHEN** 创建命令的 change 名包含变量、命令替换或其他未被有界解析器读取的形态
- **THEN** [spec-review-amendment] FF-0 SHALL fail-open 并记录 `command-unverifiable`，MUST NOT 静默放行、尝试展开 shell 或推测细分原因
...
- **THEN** [spec-review-amendment] FF-0 SHALL 输出 `command-unverifiable` context 且不设置任何 `permissionDecision`
