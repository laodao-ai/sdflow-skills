...
- **① 单一盘面：每轮全量 → 收口一次全量**。中间 fix 轮跑 **unit 全层 + 上轮失败的具体用例（⊂ unit 层）**；集成/e2e 整体推到收口。收口跑一次全量，所有判「通过」的行锚同一最终 SHA（**单一盘面语义不变**，被改掉的只是「每轮都重跑」这条对该目的零贡献的强化）。**取消「受影响层」提法**——中间轮范围 MUST 由确定信息界定，MUST NOT 由「哪层受影响」的判断界定。
...
- **⑨ red-before-green 扩展到「往既有测试补断言或修改既有断言」**。现表述只覆盖新写测试；扩展为补一条断言或修改既有断言的期望值/判定逻辑时同样 MUST 先确认它会红（当场破坏被测点验证）。
...
- **全量运行次数与 fix 轮次解耦**（结构判据，非定量阈值）：收口票的证据 schema 中，判「通过」的行全部锚同一最终 SHA，且中间 fix 轮的报告不含集成/e2e 层的「通过」证据行——只含 unit 与上轮失败用例（⊂ unit 层）。
- **`sdflow-code-review` 的复审轮数有上界**：任一 change 的 `code-review-report.md` 中存在「复审上限已达」标注，或该 change 的 checkpoint commit 中 `code-review` 标签计数 ≤ 2（首审 + 至多 1 轮复审）；超出即说明硬上限未生效。
