...
- ~~**D3 T174 fake-timeout 取整**~~ — WONTDO（设计门 Q1→B）。非整数 sec 不可达，awk 截断引入新语义错误。[spec-review-amendment]
...
- ~~T174 的修法在当前没有可达路径~~ — WONTDO（设计门拍板：不可达路径 + awk 截断引入新错误）。[spec-review-amendment]
