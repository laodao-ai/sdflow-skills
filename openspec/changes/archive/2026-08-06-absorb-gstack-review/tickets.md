...
- [x] 工作流主文档三处（编排器描述 / 代码侧质量层 / checklist 勾选项）改述为自持 scope 审计
- [x] 质量分层参考里的相关行同步改述
- [x] 编排器提示词删去第三方 skill 提法，且 `hack/tests/test_workflow_split.py` 的 needle 断言同步更新并通过
- [x] docs 下代码审技能说明、外部依赖说明、工作流总览、以及 HTML 控制台页中描述第一步的段落全部更新
- [x] 第三方 review skill 的说明文档保留，定位改述为「非运行时依赖的第三方 skill 参考」
- [x] 用不带 `--include` 限定的全量 grep 复扫一遍，逐条判定残留是「必须改」还是「合法保留」（设计审侧依赖 / 归档 / ADR / 历史文档），并在报告里列出判定依据
- [x] 本票新提法与 Task 3 在 SKILL 中使用的提法一致（报告里给出对照）
