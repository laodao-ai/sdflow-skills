...
4. **Windows 上「旧 SKILL × 新 canonical tools」不被本需求或任何替代机制覆盖**——Windows 无 symlink，`setup.sh` 用 `cp -r` 装 SKILL ⇒ SKILL 是 setup 时快照而 canonical 指活 checkout。**运行时自检结构上不可自举**（检查者只能是 SKILL 自己或 `~/.sdflow/hack/` 的 helper，二者同为一次 `cp -r` 的产物，没跑 `setup.sh` 就一起旧）；**CI 层面可测但目前未测**〔F48：`windows-recorder-smoke.yml` 已在 `windows-latest` 跑全量 pytest，失鲜回归用例记 todo，MUST NOT 表述为「结构性无测试面」〕。**已知且接受的边界，MUST 如实登记，MUST NOT 声称本 change 消灭了全部分发链失鲜。**
