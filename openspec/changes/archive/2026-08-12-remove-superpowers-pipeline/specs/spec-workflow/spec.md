...
**Reason**: 换名重立为「阶段三过设计门后连续自动跑到 merge（tickets 唯一管线）」（见 ADDED）——双轨路由与双名定位的 2 个 Scenario（「实现管线按手动确定值路由」「计划文件按轨定位且双存在即停」）随 adr/0042 删除，MODIFIED 无法删 Scenario，故走 REMOVED + 换名 ADDED；能力本身无损失。

**Migration**: 全部存续条款（无阻塞人类门、BLOCKED 异常终态、T10-choice 三级协议、/sdflow-ship 入口）逐字迁入 ADDED 需求。

### Requirement: impl-pipeline 缺省为 tickets

**Reason**: `route` 子命令与 config `impl-pipeline` 键随 adr/0042 退役（tickets 为唯一实现管线），「缺省路由到哪」失去载体。

**Migration**: 存量 `impl-pipeline` 键成无读取方的惰性键（任意取值不影响行为）；阶段三派发行为由 `impl-orchestration` 能力新增需求「阶段三派发直连 sdflow-implement（唯一管线）」定义。
