...
TG-23 命中于 D1/D4（存在被否决的合理替代方案的非显然选择；其余 D2/D3/D5-D8 为拍板后无遗留争议的决策，按避样板税不书面展开）〔spec-review-amendment D8〕：

- **D1（不搬出票步位置）** — 系统镜：不重造 `plan_first_sha` 窗口与 gate 契约，可回退性最好；用户镜：人门位置与体验不变；开发循环镜：免掉「评审 amendments 后每 change 白付一次出票」。主次判定：系统镜主导——gate 契约爆炸半径是决定性代价。
- **D4（必触发条件扩面）** — 系统镜：复用既有 T10-choice 协议，零新机制；用户镜：人门前多一层可审计记录；开发循环镜：每命中一次多付一次 strong 档复核（条件① 覆盖面已按 spec-review-report Q1 拍板选 A：合规缺席不触发，理由与实际出票矛盾视同条件③〔spec-review-amendment Q1-A〕）。主次判定：开发循环镜主导——复核成本与假绿风险的权衡是本决策核心。
