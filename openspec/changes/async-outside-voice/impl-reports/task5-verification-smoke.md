...
| 3 | 真实评审 smoke：voice 锚 reason_code = ok，非 timeout | ✅ **达成** | §3（真实 codex 跨模型，envelope `>>>0`） |
| 4 | 每站点派发/终态通知/落锚三时刻单调 | ✅ **达成** | §4（10:38:10 ≤ 10:42:32 ≤ 10:43:35） |
| 5 | fan-out 墙钟 vs voice 完成时刻，校准「重叠非叠加」 | ✅ **达成** | §5（span 316s < 叠加 443s，省 127s） |
...
| **【新发现】空闲态后台任务被回收** | 🔴 **已立 B8（P1）** | 受控对比：空闲 399s 被杀 / 活跃 660s 跑满，见 §9.2 / §9.2b / §9.2c |

> 🔴 **本票最重要的产出不是那 7 个 ✅，而是 B8**：
> 一条**直接冲击 async barrier 机制**的行为——SKILL ⑥ 要求的「让出轮次等通知」，
> 在本轮实测中**正是杀死被等待任务的那个动作**。严重度高于本票原定的任何一条验收标准。
> **它是靠「跑不通 → 不含糊过去 → 做受控对照」挖出来的，纯推理不可能得到。**
...
## 3. 真实评审 smoke：voice 锚 reason_code = ok ✅
...
## 5. fan-out 墙钟 vs voice 完成时刻：「重叠非叠加」校准 ✅
...
| ID | 内容 | 优先级 / 状态 |
|---|---|---|
| **B8** | 主 session 让出轮次转空闲后，会话内在飞的 `run_in_background` 任务被整体回收——与 SKILL ⑥ barrier 的 MUST 动作直接冲突 | **P1 / VERIFIED**（有受控对比实证） |
