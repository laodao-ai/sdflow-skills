...
- [ ] 1.2 同脚本 `exec --context-file <f> [--timeout 300]` 子命令：拼接内嵌框架 + 上下文经 stdin 喂 codex，findings→stdout，超时 exit 124，报错 exit 1 + stderr 转发〔R1·design D2 grill-amendment〕
- [ ] 1.3 「找漏 + 文件系统边界」prompt **框架**以 heredoc 内嵌 helper（不读 ~/.claude、~/.sdflow、.env/密钥；只看仓库代码；「找它漏了什么，不是重审」）；上下文由 SKILL 传 `--context-file`〔R1/R6·design D7 grill-amendment、BASE-28〕
...
- [ ] 1.5 pytest（subprocess，循 resolver 测试先例）：preflight 二态 × exec 超时(124)/报错含 auth 类(1)/正常(0)〔R1·失败模式表前 4 行〕

## 2. T25 前置：Step1 广审原生执行（→ R5）

- [ ] 2.1 `sdflow-spec-review/SKILL.md` Step1 改写：主 session 经 Skill 机制原生执行 autoplan（指令直接进主 session，非子代理转述）；原生不可用 → 模拟广审 + 报告显式标注「模拟广审（降级模式）」+ 锚行 `<!-- step1-broad-review: … -->`；保持 T20 串行纪律（checkpoint 后才 fan-out）不变〔R5·design D4·grill Q5〕
- [ ] 2.2 `sdflow-code-review/SKILL.md` Step1 同构改写（gstack /review 原生执行 + 显式降级标注 + 同款锚行）〔R5·design D4·grill Q5〕
...
- [ ] 4.4 tension 适配：有把握自动裁决记理由 / 拿不准 defer 进 issues 池 + hand-off；报告 outside-voice 段三态必填且以锚行 `<!-- outside-voice: … -->` 为固定首行〔R4·BASE-11·grill Q3/Q5〕
