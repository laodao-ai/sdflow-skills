...
- **THEN** gate MUST 保持 design-approved 有效（新鲜度按结论分域：该结论仅当其后存在触及本 change **design 域监视集**路径的提交才失鲜须重审；监视集成员由下方「监视集按角色分流」Scenario 组定义），MUST NOT 因实现提交判其陈旧而 REFUSE_START（防实现期链自锁）

#### Scenario: 监视集按角色分流——plan 落盘后 tasks.md 退出监视

design 域监视集 SHALL 为**条件集**而非固定四件套：`proposal.md` / `design.md` / `specs/` 恒在集内；`tasks.md` 的成员资格 SHALL 由 `superpowers-plan.md` 的**存在性**决定。

- **WHEN** 设计门拍板已落，且本 change 的 `superpowers-plan.md` **已存在**（完成信号权威已转移至 plan 复选框 + checkpoint 标签），其后有提交触及 `tasks.md`
- **THEN** gate MUST NOT 因该提交判 design-approved 失鲜（`tasks.md` 此时不在监视集内）；判定 MUST 仅依据 plan 文件**存在性**，MUST NOT 读取或 diff 任何文件内容
- **AND** `proposal.md` / `design.md` / `specs/` 的监视口径 MUST 逐字不变——触及其一仍照判失鲜
- **AND** plan 存在性 MUST 按 gate 运行当下求值（不逐帧回溯）；由此「先改 `tasks.md` 撞门、再手写 plan 令该改动追溯获豁免」属**显式越权同权级**（与伪造 `checkpoint(impl-review)` subject 同桶，git 留痕可审计），MUST 在脚本头注释「已知不覆盖」中声明〔grill-amendment〕

#### Scenario: plan 尚未落盘时 tasks.md 仍在监视集内

- **WHEN** 设计门拍板已落，本 change 的 `superpowers-plan.md` **尚不存在**（阶段三尚未出计划，`tasks.md` 仍是唯一任务权威），其后有提交触及 `tasks.md`
- **THEN** gate MUST 照判 design-approved 失鲜 → `REFUSE_START`（此阶段改 `tasks.md` 是真实的设计面改动，既有语义不变）
