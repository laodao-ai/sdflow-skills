...
- [ ] 截断后头段、尾段分别以严格模式解码 UTF-8 成功，覆盖一段混合 ASCII / 3 字节 CJK / 4 字节 emoji 语料在截断区间内的**每一个**切点偏移，失败数为 0
- [ ] 纯 ASCII 语料经截断丢弃 0 字节
- [ ] 存在一个变异验证：把边界回扫改成恒返回 0（即退化回按字节切）后，上述切点扫描断言**转红**——证明该测试真的承重
- [ ] 截断时 stderr 可见实际丢弃字节数，且新增输出不含 context 正文
- [ ] 密钥扫描覆盖面未缩小（`secret_scan` 仍先于截断扫整个 context 文件）
...
- [ ] 起 helper → 从外部发 SIGTERM → `ps` 验尸，无 ppid=1 的残留 runner 进程
- [ ] 退出码无回归：`0` / `124` / 其他非零码在改造后仍原样透传
- [ ] 清理时 stderr 记录被终止的 runner PID，且该输出不含 context 正文
- [ ] SIGKILL 残余在文档中被显式登记为不可消除的边界，措辞不声称「已消除孤儿」
