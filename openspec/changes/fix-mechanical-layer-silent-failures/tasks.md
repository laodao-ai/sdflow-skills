...
- [ ] 1.3 【R1】stderr 增补实际丢弃字节数；**MUST NOT** 写出 context 正文（未经出境扫描）
...
- [ ] 2.1 【R2】`do_exec()` 改后台启动 runner + 记 PID + `wait` 取回退出码
- [ ] 2.2 【R2】清理函数：`kill -TERM` → 宽限 → `kill -KILL` 兜底 → 删 workdir；trap 覆盖 `INT TERM HUP` 与 `EXIT`
- [ ] 2.3 【R2】stderr 记「已终止 runner PID N」
- [ ] 2.4 【R2】测试：起脚本 → 外部 SIGTERM → `ps` 验尸，断言无 ppid=1 的残留 runner
- [ ] 2.5 【R2】测试：退出码无回归——`0` / `124` / 其他非零码经 `wait` 后原样透传
- [ ] 2.6 【R2】文档显式登记 **SIGKILL 残余**（trap 不可执行、孤儿仍存活），**MUST NOT** 写成「已消除孤儿」

## 3. 跨平台闭环与收尾（A1）

- [ ] 3.1 【A1】`mechanical-gates.yml`（ubuntu-latest）纳入 1.4 切点扫描与 2.4 验尸测试，闭 Linux 侧未实测缺口
- [ ] 3.2 开发 checkout 跑 `bash setup.sh`（`assets/hack/` 是拷贝非 symlink，不跑就是新 SKILL 调旧脚本）
- [ ] 3.3 全套件 pytest 绿
- [ ] 3.4 跑 `hack/check_async_branch_parity.py`，确认未触碰两层 SKILL 的 async 字节等值 marker 段（Non-Goal 守卫）
- [ ] 3.5 用真实中文 diff 造 >200KB context 实跑一次 `outside-voice.sh exec`，记 rc 与锚行 `reason_code`（Success Metric 1 的度量）
