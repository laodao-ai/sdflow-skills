...
ship-gate:
  ...
  reviewed_sha: bf372ced80ee3d7a20eeac9cdd6ce81e8dd5989d
---
...
### 拍板记录〔2026-08-07 · 设计门通过〕

人对「第一性原理优化后的修订方案」整体同意（追加约束：GUIDE 保留铺进消费仓，D14 不动）。四件套已按此
重写并两轮 checkpoint（`b37f0e6` 修订 + `bf372ce` 窄复核修复），`reviewed_sha = bf372ce`。Q1–Q5 终裁：

| Q | 终裁 | 与本报告推荐的差异 |
|---|---|---|
| Q1 | **不做**机验，订正 F1 事实错误 | 同推荐 |
| Q2 | **不写自动清删代码**——告警附可复制删除命令，人一次执行（memo D18） | **翻转**（原推荐"做"；按④五问：收益=本机个位数仓×一次，与永久迁移代码倒挂） |
| Q3 | **删除 0038**，砍因入 0039 取舍段（memo D19） | 同推荐 |
| Q4 | **撤销冻结 SHALL**——`SDFLOW_HOME` 回归测试隔离既有契约，delta 只留 F42 事实性限定（memo D17） | **翻转**（原推荐"保留+收紧"；测试三层第 2 层由既有隔离契约覆盖，无需冻结承诺 ⇒ F4/F30/F31 义务面整体消解） |
| Q5 | 只订正措辞，CI 回归记 todo | 同推荐 |

另两处方案级优化随拍板生效：A5 的 `sane()` 扩面收敛为**形状级判据**（拒绝成员清单）；验收模型收敛为
**三条闭环判据**（词表 sweep 归零 + pytest 含 4 反向锚 + 三态真跑），BASE-29 表落 design、由 sweep 产出。
窄复核（`b37f0e6..bf372ce`）另抓 3 条：yq RENAMED 双反引号被 CLI 静默丢弃（实测修复后解析在场）、
`sane()` 扩面的既有 fixture 连带、tasks 行号型断言改文字定位。
