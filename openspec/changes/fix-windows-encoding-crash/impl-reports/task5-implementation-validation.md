...
全量单元 | `python -m pytest -q -rs` | 2 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`
...
单元（本 change 覆盖） | `python -m pytest -q -rs hack/tests/test_encoding_hygiene.py hack/tests/test_subprocess_encoding_contract.py sdflow-issues/tests/test_task5_delivery_contract.py -k "not upgraded_install_known_consumer_smoke"` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`

结果：18 passed，1 deselected（被排除的用例即上段已由 base 复现的既有 Windows/Git-Bash 路径失败）。

集成 | `bash -lc 'PYTHONIOENCODING=gbk bash setup.sh 2>&1'` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`
...
机械门 | `python hack/check_encoding_hygiene.py` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`

变更校验 | `openspec validate fix-windows-encoding-crash --strict --type change` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`

V-1 同构 probe | `git init "$probe" && python3 sdflow-init/scripts/init.py init --root "$probe" && PYTHONIOENCODING=gbk python3 sdflow-init/scripts/init.py update --root "$probe"` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`
...
6.5 workflow 契约 | 聚焦套件中的 `test_windows_smoke_workflow_is_persistent_and_branch_agnostic` | 0 | `5f2aa4e15eb0bce057e8e17511eb599067b13165`

该测试以整行精确匹配锁定 console 的 fail-closed encoding-hygiene 命令、setup 重定向命令及日志异常 grep；本机 Bash 无 `chcp`，真实 cp936 console 行为仍由 `windows-latest` 承载，不在本机假报通过。
...
- 本 change 相关 18 项单元测试、GBK `setup.sh` 集成、encoding-hygiene 机械门、严格 change 校验与 V-1 同构 probe 均在同一最终源码 SHA `5f2aa4e15eb0bce057e8e17511eb599067b13165` 通过；6.5 的 workflow 契约也锚定该 SHA。
