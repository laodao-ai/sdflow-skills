...
### Requirement: 新增机械门守护 reconfigure 前导块的存在性

`hack/check_encoding_hygiene.py` SHALL 对目标 glob 下每个含 `if __name__ == "__main__":` 的入口脚本，检查其是否含 stdout/stderr `reconfigure(encoding=` 调用；缺失时 SHALL 以非零退出码报告缺失清单，且该门 SHALL 独立于其余四道门运行（任一门脚本缺失不影响本门执行）。

#### Scenario: 新脚本遗漏 reconfigure 前导

- **WHEN** 仓内新增一个含 `if __name__ == "__main__":` 的入口脚本，且未在其前几行调用 `reconfigure(encoding=`
- **THEN** `check_encoding_hygiene.py` 以非零退出码报告该脚本路径

#### Scenario: 全部入口脚本均含前导

- **WHEN** 目标 glob 下所有入口脚本都已含 `reconfigure(encoding=` 调用
