...
- 补全文本模式 `subprocess`（**16 个调用站点 / 15 个编辑点**；含 1 个 `**kwargs` 动态站点）的显式编码，避免解码崩溃；`write_text()` 经复核**当前已全部带 `encoding="utf-8"`（0 处待补）**，本 change 只做核实确认并写进 spec 防回退。
...
- [x] 16 个文本模式子进程站点通过 15 个正确编辑点获得显式 UTF-8 与替换容错
...
- [x] 单元测试证据齐全并通过
- [x] 集成测试证据齐全并通过（或记「未覆盖」及判定依据）
- [x] e2e 测试证据齐全并通过（或记「未覆盖」及判定依据）
