...
`ship_gate.py` 的产物新鲜度判定 MUST 只依据**已提交盘面**（committed 产物与结论锚行），MUST NOT 将工作树 staged/unstaged/untracked 的非-openspec 改动纳入判定——与"盘面即状态 = committed 产物"地基一致〔T33/T35 定夺〕。`sdflow-ship` 编排器 MAY 在收尾以**非门禁软提示**告知"工作树存在未提交改动、gate 判定不含它们"，该提示 MUST NOT 改变 gate 的推进/拒绝语义。**merge 边界例外**：`sdflow-done` 的 merge 步 MUST 在 merge 前硬检查工作树——存在未提交的非-openspec 改动则 **MUST 停下问、MUST NOT 静默 merge**（防"忘 commit → 在缺了该随档提交之工作的盘面上 verify PASS 并 merge → 静默 ship 不完整活"）；此硬检查落 `sdflow-done`（merge 卫生前提），MUST NOT 上移进 gate 判定（不污染盘面即状态）。
...
#### Scenario: merge 边界硬检查挡未提交工作
- **WHEN** `sdflow-done` 到 merge 步时工作树有未提交的非-openspec 改动
- **THEN** merge 步停下告知并要求用户先处理（commit 或确认无关），MUST NOT 静默 ff-merge；仅当工作树对该判据洁净（或用户显式确认放行）才 merge

### Requirement: gate 熔断重试计数为编排器短时职责，不持久化下沉

阶段三熔断（同一 invocation 内同一步同步重跑一次仍无进展 → 按 UNKNOWN 停上抛）的**重试判定 MUST 由编排器在单 invocation 内短时持有**，MUST NOT 持久化为跨 turn / 跨步状态——持久化会撞"ship 零跨步状态"〔D9〕与"盘面即状态 / gate 零副作用"〔adr/0006〕三条红线（"重跑无新产物"本质无盘面差异，gate 无法零副作用地区分首跑 vs 重跑）〔T26 定夺：持久化不下沉〕。**触发判据 MUST 硬化为具体盘面对照**（非模糊计数）：编排器重跑一步**前** MUST 检查 `HEAD` 与该步报告是否**自本 turn 上次跑后未推进**（HEAD 未移动 + 报告 mtime/sha 未变）；未推进即判无进展、停上抛人工。保留一个不可消除的 within-invocation "before 快照" bit（架构下限），但从"数重试次数"降级为"做一次具体 git 对照"，缩小 LLM 漂移面。

#### Scenario: 熔断触发靠盘面对照而非计数
- **WHEN** 同一 invocation 内某步重跑，且重跑前 `HEAD` 与该步报告自上次跑后均未推进
- **THEN** 编排器据此判无进展、触发熔断（UNKNOWN 停上抛人工）；MUST NOT 依赖模糊的"我数到第几次"，MUST NOT 写任何跨步/跨 turn 的计数或快照状态文件；跨 invocation 重调 `/sdflow-ship` 时判定不延续（与"停即停、重调即续"一致）
