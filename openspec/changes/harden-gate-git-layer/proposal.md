...
> 🔴 **砍的是枚举，不是监视集。** 监视集（只盯四件套 / 只盯非 `openspec/`）是**承重的**——它才是「实现期改源码不该让设计门失鲜」的来源。裸 `reviewed_sha == HEAD` 已实测证伪：**实现期每个提交都在动 HEAD** ⇒ 设计门从实现的第一个提交起永远失鲜，等于把 `fix-design-gate-freshness-proxy` 修的缺陷退回去。
