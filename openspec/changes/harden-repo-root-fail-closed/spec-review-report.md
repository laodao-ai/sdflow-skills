...
│ [需拍板] Q2  是否把「消除镜像手工同步」升为本 change 范围内（codex X6） │
...
### Q2（需拍板）— 镜像手工同步要不要在本 change 内消除

codex X6：`determinism-guards` 的 guard 覆盖 **37 个三向 + 24 个两向** helper，每次修复都要手工三改，AST 测试只能发现漂移、不能消除漂移源。仓内**已有现成模式**：`hack/sync_principles.py` 的「单一源 + 机械注入 + `--check` 门禁」。

**我的推荐：不纳入本 change，但立即记 todolist 并在 design 里显式指向它。** 依据：本 change 的 scope 是「一个完整阶段结果 = 修好仓根解析的信任边界」；把 61 个 helper 的分发机制改造塞进来会让 Q1 的返修失焦。代价：本次仍需手工三改（可接受，AST 门会拦漏改）。**这是「唯一合理 defer = 依赖的模块尚不存在」之外的一次判断，请你确认。**
...
| # | 内容 | 去向 |
|---|---|---|
| **F1** | 镜像手工同步的分发机制改造（61 个 helper） | todolist，待 Q2 拍板 |
...
返修范围 = Q1（四条 critical 合并重画信任边界）+ D1–D6（自动决策，直接改）+ Q2/Q3/Q4 待你拍板。
