...
- [ ] 命令函数内的 `repo_root` 调用归零；全脚本 Call 节点从 **19** 降到 **3**（三份入口各一），
      统计手段用 `ast.walk`
- [ ] `--root` 未指定与显式指定两条路径行为可区分（未指定 → 探测；显式 → 先校验）
- [ ] `repo_root` 的 docstring 与新架构一致（现文描述的是旧架构，ADR-5 后失真）
