...
- **C1 范围只覆盖源仓后续项** — T233–T238、T240–T242 与 T132 的前置订正纳入本 change；T239 的下游 `sdflow-init update` rollout 明确排除。**证据锚**：用户 2026-07-27 明确指示；`openspec/issues/todolist/2026-07-todolist.md:92-102`。
...
- **D3 未判定的 FF-0 调用保留 fail-open，但带上下文审计** — 采用无 `permissionDecision` 的 `additionalContext` JSON 输出。依据：不解析无界 shell 语法、不越权自动批准、使模型和 transcript 都看见原因；**砍掉的候选**：返回 `allow`，理由：会跳过宿主原生权限提示。
