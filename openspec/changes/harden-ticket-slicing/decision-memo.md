...
- **D4（必触发条件扩面）** — 系统镜：复用既有 T10-choice 协议，零新机制；用户镜：人门前多一层可审计记录；开发循环镜：每命中一次多付一次 strong 档复核（条件① 对合规缺席小修的覆盖面见 spec-review-report Q1，待拍板）。主次判定：开发循环镜主导——复核成本与假绿风险的权衡是本决策核心。
