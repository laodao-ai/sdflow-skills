...
- **Δ 归属口径（读侧全局按 session 跨 change 分组，设计门 Q1 拍板=A）** [spec-review-amendment]：retro 先扫描全部 change 目录（活动 + 归档）的 token-log.jsonl，按 `session` **全局**分组；组内相邻 `anchor=true` 行差分归后一行的 `step`（attribute-to-next）——同一 session 出现在多个 change 文件时，后一文件首行对前一文件末行差分、Δ 落该行所在 change，消除跨 change 双计数；仅 session 的**全局首行**（未在任何其他 change 文件出现过更早行）全额计入该行 step。写侧契约不动（usage 恒为 session 累计值），口径纯读侧、可迭代。tokens 列旁恒加脚注「数值为各会话累计口径聚合，tickets 管线下多为独立短会话的首行全额之和，非严格阶段增量」。
...
- [token-log 与墙钟的 change 归属毛边（跨 change session / 归档后 checkpoint）] → 归档后无落点静默跳过维持；跨 change session 用量由读侧全局按 session 分组差分消除双计数（Δ 归属口径，设计门 Q1 拍板=A）。[spec-review-amendment]
- [change 收尾（verify/archive/merge）token 系统性不入账] → `sdflow-done` 不走 checkpoint，最后一次 checkpoint 之后的用量对**每个** change 缺失——非低概率边角，是稳定同向偏差（tokens 列系统性偏低，趋势参考仍可用，脚注已覆盖）。设计门 Q2 拍板=A：本 change 接受、不扩 scope 动 sdflow-done；阶段 2 评估在 sdflow-done 加终态快照（roadmap 2.A.5 已记）。[spec-review-amendment]
