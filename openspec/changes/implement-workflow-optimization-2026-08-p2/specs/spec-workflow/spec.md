...
- 条件判「本轮不派」的镜 MUST 照落 lens-metric 锚行（`runner="none"`、`findings=0`，成因 `condition-not-met`）——跳过 MUST 可审计，MUST NOT 以不落锚的方式静默消失。
...
- **THEN** 该镜不派发，本轮报告仍落其锚行 `runner="none" findings="0"`（成因 condition-not-met），报告可见一行跳过说明
