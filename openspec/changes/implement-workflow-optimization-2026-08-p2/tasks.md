...
- [ ] 3.2 两评审 SKILL roster 段落地处置：降采样镜（草案：code-review history、spec-review grounding——grounding 处置待设计门重议，见 spec-review-report Q3）加派发条件行，**并给出条件阈值的具体取值与判定命令**（如 `git diff --numstat` 行数阈值 / `--diff-filter=R` rename 检测，随 roster 段落盘，不留「大规模」定性词）〔spec-review-amendment〕；条件跳过轮落锚 `runner="none" findings="0"`（condition-not-met 落点待 Q1 拍板）+ 报告一行说明〔R-roster〕
