...
- [ ] 2.0 [spec-review-amendment] cwd 守卫（两子命令起手 git remote 判定，非本仓 fail-loud
...
- [ ] 2.2 gstack 采集器：既有 checkout `git fetch origin` + `merge-base --is-ancestor` 锚祖先
...
- [ ] 2.3 bare 缓存采集器（matt / superpowers 共用缓存层）：`clone --filter=blob:none --bare`
...
- [ ] 2.6 facts JSON 输出（per-source status/reason/commits/changed_paths/drift 清单，落
...
- [ ] 3.1 anchors 语义测试：首轮初始化（per-source：gstack 天然锚真 delta / 其余当前态基线
      [spec-review-amendment]）/ yq 失败硬停 / 值缺失无锚（tmp_path + 假 HOME）（R1）
- [ ] 3.2 advance 门测试：报告缺失拒推锚（内容不变 + 非零退出）/ 报告缺 facts sha 拒推锚
      [spec-review-amendment] / 报告在场正常推进 + `last_run` 更新 / degraded 源锚逐字不变
      [spec-review-amendment]（R1）
- [ ] 3.3 采集器降级矩阵测试：单源不可达其余照采（git 命令 stub 注入）/ 单源挂起超时其余照采
...
- [ ] 3.4b [spec-review-amendment] superpowers 字段追踪测试：构造 bare 仓含 marketplace.json
...
- [ ] 4.1 SKILL.md 编排正文：collect → 模型读 facts 写报告（`reports/<UTC时间戳>.md` 不覆盖
...
- [ ] 4.3 入池衔接条款：人拍板「吸」→ recorder `add` 显式 `source_change`（报告内预生成命令
...
- [ ] 5.1 `sdflow-upgrade/SKILL.md` 追加第 5 步提醒段：读
      ...
      缺失/不可解析静默跳过，零网络（R6）
- [ ] 5.2 README「Skills 列表」加 `sdflow-upstream-watch` 行（显式注明单仓专用，非通用铺设类
