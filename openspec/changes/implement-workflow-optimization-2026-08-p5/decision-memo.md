...
generated_at: 2026-08-12T15:38:43+08:00
decision_hash: f847f292383a
---
...
- **分批提问的批内连带作废** — 概率低（独立批仅收互相独立问题，误判独立性才发生；依赖链
  走 D3 ② 整链呈现，链头改判时下游重提是设计内行为非作废）/影响小（当场撤回重问一句）/
  完美成本高（逐问依赖分析不值）；**为何接受**：残余仅剩 ① 中误判独立性的边角。
...
D1/D2/D4 无 TG-23 命中（流程/编辑类，无架构性分岔的多方案选择；Q1 界线的载体选择已在 D1
记录依据与代价：references/ 旁文件默认不加载 vs 文末附录仍占 SKILL 加载 token）。

〔2026-08-12 设计门 Q1 拍板补记，人确认〕**D3 判 TG-23 命中**，三镜简表：

- **系统镜**：纯 SKILL 条款 + spec delta 改动，无代码耦合，`git revert` 即回退；
- **用户镜**：减少拷问往返轮次；整链呈现使人拍链头时可见下游影响（实测样本：本 change
  拷问期 Q1 载体子问题因未随链头同呈现多耗一轮）；
- **开发循环镜**：条款固化免每 session 重新养成习惯，分批/整链协议跨 change 复用；
- **主次判定**：用户镜主导——本决策本质是人机交互协议，人的注意力效率是第一收益。
