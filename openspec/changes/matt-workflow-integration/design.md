...
- **候选**：A. 两 skill（出票 + 执行，与 gate 两状态 1:1）；B. 单 skill 双模式、出票**落盘即返回**；C. 单 skill 直通（出票后同调用续执行）。
...
- **模式路由信号（确定性，非模型判断）**：RUN_PLAN（plan 文件缺，ship_gate.py:722-724）→ 出票模式；CONTINUE_IMPL（携 done_tasks，:750-752）→ 执行模式。skill 内部不自判模式。

### D2 ship 接入：原地条件路由，不 fork（三镜全票）
...
### D3 gate 处理：试验期零改动外衣；否决配置化；emit 串归 Phase B（三镜全票）

- 完成判据契约 = 文件名 `superpowers-plan.md`（:722）+ `### Task <n>:` 标题集（fence-aware，:730-739）+ checkpoint 标签 ∪ 复选框双通道（:740-752），票体内容不设限 → tickets 穿外衣零改动兼容，CONTINUE_IMPL 的 done_tasks resume 语义原样可用。
...
### D4 机制取舍（三镜全票，keep/drop 判据 = sdflow 已有结构机制覆盖者砍、确定性信号与上下文经济学机制保）
...
| 砍 | task-brief 抽取层 | 行为级票文（禁代码/路径）远小于带码 task，票即 brief；对抗镜证伪了「票变粗 brief 更大」的反向解读 |
| 砍 | pre-flight 批量问人 / plan-mandated 问人 | 机械冲突 gate 已查（:727-739）；裁决走 ship T10 三级协议（SKILL.md:23），阶段三无人类门 |
| 保 | 状态词表 DONE/DONE_WITH_CONCERNS/NEEDS_CONTEXT/BLOCKED | 枚举状态 = 确定性分支信号；BLOCKED 与 ship BLOCKED_UPSTREAM 上抛同构；NEEDS_CONTEXT handler 重定义为盘面自答（design/specs/票文），答不出 T10/停 |
| 保 | file handoffs（report file / review-package diff 文件）〔T125 实例〕 | 上下文经济学与票粒度无关（42k 字符 dispatch 事故教训）；review-package 类等价物以 skill 内几行 git 命令契约实现（不复制 superpowers 脚本） |
| 保 | 每票双轴审 + fix→re-review 环 | Standards 轴（仓标准 + smell 基线 + **注入 code-checklists/domains = 注入点 B，经 resolve-workflow.sh 解析**，落 dispatch 模板必填槽而非 prose 叮嘱——防注入点丢失前科）+ Spec 轴（票文验收 + R-ID）；各 <400 词封顶 |
...
- 三段分流改 workflow.md 阶段一（图 :13-21 + 步骤表 :68 explore 行），wayfinder 入口与降级（缺装→explore）一并写入；衔接契约三条落 `ff-generation-constraints.md` 新节（FF-0 同级、条件 = change 源于 wayfinder map）：ff 起手逐区读 map（Destination→proposal 动机+D-5；**Decisions-so-far 逐票 zoom 决议全文**，防 ff:100「prefer making reasonable decisions」对已决项重决歪；Out-of-scope→D-3 假设）、TG 判命中前置 chart 写 map Notes、proposal 回链 map。
...
- 护栏：MUST NOT 整跳 grill（memory: grill-not-skippable）；瘦跑仅作用于有 resolved 票对应的分支。
...
- 判据三条结构、**定性人读拍板**（n=3-5 不设数字阈值，adr/0009 小样本警告）：① retro per-change impl Δ 方向性下降；② 冷层 Critical/严重 findings 与 done verify FAIL 不升；③ 护栏哨兵——冷层捕获「本应被每票审拦住」的严重项占比不恶化（恶化 = 熔断，停试点回退）。
- 变量控制：试验期 implementer 档位钉死 mid（不叠加降档实验；「plan 含码→最便宜档誊抄」降档通道随预写代码消失而失效，model-tiers 判据重标另议）。

### D9 出票粒度人工话语权（开放问题③推荐）

推荐：quiz-the-user 职能并进**设计门**——design.md 可选「切片建议」节（初步票划分 + 阻塞边草图），spec-review 顺带审、设计门一次拍板；出票模式以该节为**建议**输入（无则自主出票），粒度争议走 T10。零新增人类门（adr/0004），人对粒度的话语权前移到既有拍板面。
...
graph LR
    ...
    SHIP -->|RUN_PLAN·键=tickets| TK[sdflow-implement 出票模式]
    SHIP -->|RUN_PLAN·缺省| WP[superpowers:writing-plans]
    TK -->|落盘即返回| PLAN[(superpowers-plan.md<br/>票文+marker+复选框)]
    PLAN -.marker.-> SHIP
    SHIP -->|CONTINUE_IMPL·marker=tickets| EX[sdflow-implement 执行模式]
    EX --> IMP[implementer 子代理/票<br/>TDD+checkpoint 双写]
...
sequenceDiagram
    ...
    S->>G: gate(change)
    G-->>S: RUN_PLAN(plan 缺)
    S->>I: 出票模式(config 键=tickets)
    I-->>S: superpowers-plan.md 落盘(票+marker) 即返回
    S->>G: gate → fence/标题/重号三道校验
    G-->>S: CONTINUE_IMPL(done_tasks=∅)
    S->>I: 执行模式(marker 路由)
    loop frontier 串行(每票)
        I->>W: fresh implementer(票文+Global Constraints)
...
| 失败模式 | 探测 | 降级/处置 |
|---|---|---|
| matt 语义源缺装（to-tickets/implement/code-review/tdd） | 出票模式起手检查 skill 目录 | 显式停并提示；config 未开 tickets 时本不可达（缺省仓零暴露） |
...
| 勾框/标签双写半态 | gate 双通道并集不假卡；执行模式逐票核对双信号 | implementer 契约钉死双写；单边缺失时执行模式补齐并记录 |
...
| implementer BLOCKED 无法盘面消解 | 状态词表 | 停并上抛（同 ship BLOCKED_UPSTREAM 语义），不静默跳票 |
...
- **adr/0006(b)**：步序判定全在 gate；出票落盘即返回保插入点；next 提示串失真以链序显式声明消歧（状态判定未失真），Phase B 根治。✓
