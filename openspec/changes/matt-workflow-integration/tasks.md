...
- [ ] 1.3 执行模式契约：frontier 串行（禁并行 implementer）、fresh implementer/ticket（TDD at pre-agreed seams + 定期 typecheck + 末尾全套件 + checkpoint `<change>:task<N>-<slug>` 与勾框**双写**）、状态词表四值处置（NEEDS_CONTEXT 盘面自答边界：design/specs/ticket 文本，答不出 T10/停；BLOCKED 停并上抛）、report file/review-package 文件交接〔T125〕、Reviewer ⚠️ cannot-verify 项编排层亲自消解〔R3, R4〕验证：implementer dispatch 模板含双写命令原文与 report 路径契约
