...
| 步 | 代码位 | absent(=None) 下游 | 退出码 | next（编排器动作） |
|---|---|---|---|---|
| design | `decide()` L695-700 | `sr_state=None` → `design_ok=False` → REFUSE_START | **3**（拒绝码） | 停，补设计门 |
| code-review | `decide()` L760-771 | `cr_state=None` → STEP_IN_PROGRESS | **0**（可推进码） | **重跑本步** `sdflow-code-review` |
| verify | `decide()` L795-814 | `v_state=None` → STEP_IN_PROGRESS | **0**（可推进码） | **重跑本步** `sdflow-done` |
...
> **归档侧影响（订正原「只影响 live」事实错误）**〔grill-amendment Q2〕：`archived_verify_state`（`decide()` D3 短路的 SHIPPED 判据源）**共用**同一 `parse_ship_gate_frontmatter`，改判 absent 对归档侧行为**亦变**（非无影响）——详见 ADR-4。目标态论证结论：live/归档**两侧同向 fail-safe**，原「T74 只影响 live」为事实错误、已订正。
...
### ADR-4〔grill-amendment Q2〕：归档侧影响按 producer 契约 + 目标态论证，两侧同向 fail-safe（升 `adr/0011`）

**订正事实错误**：改判 absent **不只影响 live**——`archived_verify_state`（D3 短路 SHIPPED 判据源）L196 共用同一 `parse_ship_gate_frontmatter`，「首行 `---` 无闭合」归档报告的处置随之变：
...
**结论**：live 侧漏闭合 → absent → REFUSE_START/重跑（不放行）；归档侧漏闭合 → absent → 回退 inline 扫空 → `none`（不 SHIPPED）——**两侧同向 fail-safe**。初版所谓「归档侧回退 inline 判假 pass」需「`---` 打头 × 正文 inline PASS 锚」杂交形态，**无 producer 会产出**（未来 producer 不写 inline、旧 producer 首行 `#`），须手工伪造归档 = 显式越权（`adr/0008` 立场，git 可审计）→ 登记入 `ship_gate.py`「已知不覆盖」。
...
- **[归档侧共用核心行为变化]**〔grill-amendment Q2〕 改判 absent 经共用 helper 波及 `archived_verify_state`：「首行 `---` 无闭合」归档报告由 `none`(fail-safe) 变为 absent→回退 inline dual-read。→ **Mitigation**：ADR-4 目标态论证证两侧同向 fail-safe（漏闭合归档无 inline 可扫→`none`）；假 pass 杂交形态无 producer 产出、登记 `ship_gate.py`「已知不覆盖」；加目标态归档回归测试（tasks 3.5）钉死此行为。
