...
`[grill-amendment]` bug/todo 新写 ID SHALL 统一符合 `[A-Z][1-9]\d*`，默认 prefix 分别为 `B`/`T`，公开的自定义 `--prefix` 保持兼容且只接受一个 ASCII 大写字母。ID 的语义身份由大写 prefix + 十进制数字值决定，不包含 pool；reader MUST 把 `A007`/`A7` 等同号异形视作冲突并 fail-closed，writer MUST 只接受 canonical spelling。ID SHALL 在 bug+todo 两池仓级全局唯一；显式 ID 查重、自动 max+1 分配与 `next-id --prefix X` 的计算 MUST 在同一个 snapshot lock 内读取两池全集，MUST NOT 只扫当前 pool。`next-id` 命令只提供 advisory 值，不构成 reservation。

所有会形成权威仓级 snapshot 或修改 dated 文件、`issues/INDEX.md`、`issues/batches.md` 的 recorder 命令 MUST 以 `O_CREAT|O_EXCL` 获取仓级 exclusive snapshot lock `openspec/issues/.recorder.lock`。只读命令 SHALL 从首次业务文件读取持有到全部输入已读取、解析、校验并固化成不再访问文件的内存 snapshot，随后先释放再格式化/输出；mutation SHALL 从首次业务读取持有到最后一次原子替换完成。两类命令均在正常/异常退出的 `finally` 路径释放。至少包含 bug/todo `scan`、`next-id`、add/set-status/triage，以及 issues reindex/sweep、batch query/mutation/rename。锁内容 SHALL 记录 PID、命令、开始时间与高熵 capability token。锁冲突 MUST 在任何业务 snapshot/写盘前立即非零失败并显示 owner，但 MUST NOT 显示 token；实现 MUST NOT 等待、自动按 TTL 偷锁或忽略冲突。目标文件落盘 MUST 继续使用同目录唯一临时文件 + `os.replace`，写前异常不得改变旧文件。

`[grill-amendment]` lock 只提供 cooperative isolation，MUST NOT 声称可阻止不遵守协议的编辑器/脚本/Git 修改。`sdflow-init` SHALL 提供 canonical runtime-ignore snippet，并在 init/update 时向消费仓根 `.gitignore` 幂等合并 `/openspec/issues/.recorder.lock`；已有其它 ignore 内容必须保留，重复 update 不得重复插入。本仓 `.gitignore` SHALL 同步 dogfood。当前不存在的 canonical ignore 资产必须由本 change 新增。

`[grill-amendment]` 顶层 snapshot/mutating CLI SHALL 是唯一 lock owner；它启动的 scan 或 mutating 子进程 SHALL 通过专用环境变量接收 token，只有在 token 与当前锁文件匹配时才作为 participant 进入同一锁域。participant MUST NOT 再次 acquire 或 release；非 owner 直接调用、缺 token 或 token 不匹配时仍须正常竞争锁或 fail-closed，MUST NOT 绕过互斥。owner 释放前 MUST 再次核对锁文件 token，只能删除自己仍拥有的锁。`O_EXCL` 创建成功后即视为 occupied；竞争者读到空白/部分 metadata 时仍须失败，不得据此偷锁。独立 reader MUST 真正 acquire；仅在读取前后检查 lock 是否存在不满足 snapshot 隔离。
...
- **THEN** 旧 owner 在 release 时检测 ownership lost，不删除新锁，并以显式诊断结束
...
- **THEN** 后续 writer MUST fail-loud 并给出“核验 owner 已退出后删除该 lock 再重试”的恢复指引；MUST NOT 自动删除可能仍属活进程的 lock
...
`scan` SHALL 对每个 dated 文件至多执行一次文件读取与一次 document parse，并从同一 parse result 完成 frontmatter/legacy merge、ID/字段校验、block relation 与 JSON 输出；MUST NOT 为 arity、重复 ID、rows、blocks 分别重复切分同一文件。`issues.py batch rename` SHALL 在一次命令中每 pool 至多执行一次 `scan --json`，retag 时同步更新已读的内存 snapshot，并把该 snapshot 传给 reindex 核心生成 INDEX/batches；MUST NOT 在 rename 成功后为 reindex 再次读取两池。该约束不把多文件 rename 宣称为事务；`[grill-amendment]` rename 只有在 dated files、batch key、INDEX 与 batches 派生状态全部收敛后才可 exit 0，任一步失败均非零并允许原命令按当前盘面识别阶段后重试。
...
- **THEN** bug/todo 的 `scan --json` 各调用一次，reindex 使用 retag 后 snapshot 生成输出，不再出现第二轮两池 scan
...
- **WHEN** 重试 `batch rename old new` 时 registry 已无 `old` 且已有 `new`，snapshot 中也不存在仍标记 `old` 的 item
- **THEN** 命令把该盘面识别为 rename 主写已完成，不因 source key 缺失而退出；它补跑 reindex，成功后 exit 0，失败仍非零

#### Scenario: retry 不静默合并冲突 batch `[grill-amendment]`
- **WHEN** 重试 snapshot 中 `old` 与 `new` registry entry 同时存在，或 registry 仅有 `new` 但仍有 item 标记 `old`
- **THEN** 命令 fail-closed，列出冲突 registry/item 证据，不 retag、不 merge、不写派生输出
