...
- 新 SKILL.md 的相位协议与 sdflow-spec **共享同一套骨架词汇与节奏**（起手判定 → 增量落盘 →
  收敛 → 生成 → 收尾门），**但不是「逐节同构」**〔spec-review-amendment SR-35 · 窄复核 NR-5〕——
  实测至少五处实质分叉，**逐条见下方「与 sdflow-spec 的实际分叉表」**，其中两处（指令密度策略、
  提议制防线层数）是**未论证的分叉**，不是有意设计。
...
**第零步：重入探测**（独立标题，**在相位 A 之前**——与 sdflow-spec 的「第零步」同位）〔SR-36 · NR-6〕→
...
入口（人触发 /sdflow-roadmap；想法未成形 ⇒ 建议先 opsx:explore 再回来）
  │
  ▼
第零步：重入探测（扫 openspec/roadmaps/*/memo.md 的「状态：DRAFT」）〔SR-36 · NR-6〕
  ├─ 命中 1 个 ──▶ 呈现包名 + memo 摘要 ──▶ 人选「继续」──▶ 回相位 B（既有包，continue）
  │                                        └─ 人选「新开」──▶ 既有 draft 原样保留，继续往下
  ├─ 命中 ≥2 个 ─▶ 逐个呈现，人选其一（同上）
  └─ 未命中 ─────────────────────────────────────────────┐
                                                          │
  ▼◀────────────────────────────────────────────────────┘
...
```
                    ┌──（放弃 ⇒ create：先复述路径再删包目录；continue/replan：不自动删、只记 task-log 一行）〔SR-5·NR-2〕
                    │
absent ──A收束，B起手：定名 + 生命周期判定(create/continue/replan) + 建目录 + 草稿memo──▶ B-draft
                                                                                        │
   （直接生成路径：absent ──gate-0过∧无信号──▶ 生成中(建目录) ──▶ 三件套就绪）          │拷问收敛
                                                                                        ▼
定稿包 ◀──收尾四项过（判定点③）+ memo 置 FINAL ── review+处置（判定点②）◀── 三件套就绪 ◀──C生成── B 收敛（memo 仍 DRAFT）
```
