...
1. **先拍 Q1 / Q2**（只有你能定，各 2 分钟）。
2. **按修订清单改四件套**（27 条可直接执行，无需再决策；其中 SR-2 / SR-3 / SR-4 / SR-5 /
   SR-7 / SR-13 / SR-16 / SR-17 / SR-18 是**必改**，不改会在实现期确定性翻车）。
   改动处标 `[spec-review-amendment SR-N]`。
3. **跑一次窄复核**（只审增量）——本轮改动量大，且 🔴 **拍板批准的盘面必须与镜子审过的盘面一致**：
   镜子审的是 `c78c02c`，修订后是新盘面，findings 只针对旧盘面。
4. **再拍板**，随后按 `sdflow-spec-review` 的拍板回写协议把
   `ship-gate.design_approved` + `reviewed_sha` **同一次写入**本报告头部 frontmatter。
   ⚠️ 若修订在拍板前发生（必然），**MUST 先把修订单独 checkpoint 提交、取得其 sha，再回写锚**——
   否则第一次跑 gate 就会判 design 失鲜、当场 `REFUSE_START` 自锁。

**为什么不是「小修即可放行」**：三条致命里，两条是**你在错误依据上拍的板**（C1/C3），
一条是**照做必然报错**（`OBSOLETE`）；再加上 SR-7 那个面级模式（标「保留」的节里嵌着待删机制、
且机械门查不到）——这些不是 nitpick，是会在实现期确定性发作的东西。

**方向本身没有问题**：契约面干净（无悬空 SHALL、三条 Migration 全真）、
scope 判断符合本仓基准 4、早期草稿的静默缩水已被相位 B 主动纠正。改完再审一轮就能放行。
