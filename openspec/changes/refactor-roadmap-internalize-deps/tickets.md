...
- [x] 全量 grep 残留扫描（不带 `--include`，词表见 6.1）跑过，命中逐条过滤，非白名单残留为零（6.1）
- [x] 构造 footage fixture 包，跑续跑 / 重入 / 收尾三条路径：不报错、不迁移文件、不新增票、未决票不阻塞收尾、至多一行冻结提示；演练后临时包已删除（6.4）
- [x] 真实单文件包 `issues-triage-2026-08/` 走 continue 判定：不报错、收尾 ② 对缺失文件判「不适用」（6.5）
- [x] 终审场景核对清单 6×3 逐格核对完成，每格在新 SKILL.md 中有明确答案（6.8）
