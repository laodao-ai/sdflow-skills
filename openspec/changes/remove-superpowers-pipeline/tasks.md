...
- [ ] 2.1 `PLAN_FILENAMES` 缩为 `("tickets.md",)`（resolver 函数形状保留）、删双存在判 UNKNOWN 分支、删旧名收尾票 grandfather 分支、RUN_PLAN reason / UNKNOWN 表 / 文件头注释的双名表述改单名〔R2/R7/R8〕
- [ ] 2.2 测试同步：`test_plan_resolver.py` 旧名探测 / 双名 UNKNOWN / 改名迁移窗口用例退役；`test_superpowers_track_regression.py` 整文件删除；`test_harden_sdflow_spec_followup_closure.py` fixture 改名 `tickets.md`〔R2〕
...
- [ ] 5.3 全仓测试绿：`/usr/bin/python3 -m pytest`（含保留半场、gate 回归、hack 守卫全量）
