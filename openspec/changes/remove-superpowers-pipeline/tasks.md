...
- [ ] 4.5 `specs/yq-yaml-operations/spec.md` delta（评审补齐）随归档同步核验：R3/R5/R6 的 impl-pipeline Scenario 删除落主 spec；主 spec Purpose 脚本枚举去 `impl_route.py`（R12 惯例：Purpose 随 change 直接改主 spec）；`test_yq_wrapper_consistency.py` 成员按 Q2 拍板结果同步〔R11；spec-review-amendment〕
