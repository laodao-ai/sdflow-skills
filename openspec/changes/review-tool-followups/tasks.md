...
- [ ] 1.4b 〔Q-D1·NEW-2·**设计门已批准 A**〕`_deregister_hook_in_settings`（init.py:313）改**原子写**（写 temp 文件 + `os.replace` 原子落）——T44 把该写接进 `/sdflow-upgrade`、常与活跃 Claude Code 会话并发写 `~/.claude/settings.json`，非原子写有丢更新/撕裂 JSON 风险，且崩后坏 JSON 被 retire 的 fail-safe 读静默 no-op 掩盖。补 1 条测试（撕裂/并发场景）。
