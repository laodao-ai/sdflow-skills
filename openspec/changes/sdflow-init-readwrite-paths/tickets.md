...
- [x] `_atomic_write_settings()` 使用 `tempfile.mkstemp` 替代固定名 tmp，mkstemp 在外层 try 内
- [x] 测试：验证 tmp 文件名非固定（mock mkstemp 或检查 os.replace 被调用时的源路径前缀）
- [x] 测试：mkstemp 失败时返回 False 而非裸抛（mock mkstemp 抛 OSError，断言返回 False）
...
- [x] 新增 `_detect_duplicate_top_keys()` 函数（行级扫描，encoding=utf-8-sig，except (OSError, UnicodeDecodeError)）
- [x] 在 `lint_config()` 的 `_yq` 调用之前调用，重复键追加 reason
- [x] 测试：构造含重复顶层键的 config.yaml，验证 `lint_config()` 返回含「重复」的 reason
- [x] 测试：非 UTF-8 config.yaml 不让 `_detect_duplicate_top_keys` 裸抛
- [x] 测试：BOM config.yaml 首键正确识别
...
- [x] `ensure_global_hooks()` 末尾检测 `os.path.isdir(~/.codex/)` 时追加告警行
- [x] 测试：mock `~/.codex/` 存在，验证输出含 `⚠` 告警；不存在时无告警
