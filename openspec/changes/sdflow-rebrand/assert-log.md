...
| 托管区块内容已是新名 | `grep -n "sdflow-spec-review" CLAUDE.md AGENTS.md openspec/INDEX.md` | 命中 5 处（CLAUDE.md ×3、AGENTS.md ×2、INDEX.md ×1），均为新名 |
...
重跑同一命令，命中总行数从 128 降到 121（减少 7 行，与上表 7 处修复一一对应）；`pattern 无命中直接落 "clean"` 分支的
