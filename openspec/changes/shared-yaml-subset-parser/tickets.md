...
- [x] `setup.sh` 运行后输出每项依赖一行状态（✓/✗/·）
- [x] yq 已安装但为 kislyuk/yq 时输出警告 + 正确版本安装指引
- [x] yq 未安装时输出 ✗ + 三平台安装命令
- [x] 必要依赖缺失时在末尾汇总安装指引，但不中止 setup.sh
- [x] 既有 python3 检测逻辑已从原位迁入 `check_dependencies()`，不重复
...
- [x] `read_metrics_enabled` 改为通过 `_yq()` 读取
- [x] `_yq()` 封装含 `shutil.which` 检测 + `--version` 身份校验 + 进程内缓存 + fail-loud
- [x] 两份 `anchor_lint.py` 副本字节一致
- [x] `pytest sdflow-init/assets/workflow/tools/tests/test_anchor_lint.py` 全绿
- [x] 零依赖声明注释已更新
...
- [x] 全部手搓 YAML 解析函数已删除并替换为 `_yq()` 调用
- [x] `_parse_model_tiers_block` 的业务逻辑在 yq 返回的 Python dict 上执行
- [x] `_set_schema_key` 写操作通过环境变量传值（R13）
- [x] `lint_config` 入口前有 yq 可用性检测
- [x] 零依赖声明注释已更新
- [x] `pytest sdflow-init/tests/` 全绿
