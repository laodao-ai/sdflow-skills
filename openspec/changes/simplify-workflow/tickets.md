...
- [x] impl_route.py 中缺配置/缺键/空值/非法值的 return 已改为 `"tickets"`
- [x] impl_route.py 中 `_cmd_route` 展示折叠逻辑已对称翻转
- [x] `openspec/config.yaml` impl-pipeline 注释已更新 + wayfinder 引用已删
- [x] 两份 `config.template.yaml` 注释已改为"缺省 tickets"
- [x] `sdflow-ship/SKILL.md` 缺省描述已更新
...
- [x] `claude-section.md` 已更新为单轨描述
- [x] `CLAUDE.md` 已删除四入口选择规则段、sunset 条件段、grill-with-docs 段
- [x] `AGENTS.md` 已删旧双轨/手动触发/ff→grill 引用
- [x] `openspec/workflow/` 本地 pin 已处置（删除或同步）
- [x] companion 文档中 RUN_SOP/embedded-test-sop/wayfinder/分支 B 引用已清理
- [x] README.md Skills 列表无 embedded-test-sop
