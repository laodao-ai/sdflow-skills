...
**决策后果 SHALL 按三镜展开**——系统镜（对系统本身：耦合 / 依赖 / 复杂度 / 可回退性）、用户镜（对最终用户使用：体验 / 可感知行为 / 干扰）、开发循环镜（对后续开发过程循环：心智负担 / 是否靠人 / 流程开销 / 复用性）——并 MUST 附**一句主次判定**（对当前这个决策，三镜哪个更重要、为何据此选定），MUST NOT 只罗列三镜后果而不判主次。触发深度 SHALL 分层：行为层每个决策都按三镜 + 主次分析；书面写入报告的三镜 + 主次 MUST 覆盖命中 TG-23（≥2 合理方案 / 非显然设计）的决策点，琐碎决策（无 ≥2 合理方案）SHALL NOT 被强制写满三镜段（避免样板税）。

#### Scenario: sdflow-spec-review 遇到 ≥2 合理方案
- **WHEN** 某评审镜发现一个有 ≥2 合理方案或核验不了的事实的决策点（命中 TG-23）
...
- **WHEN** 阶段三 code-review 遇 ≥2 方案、有客观判据自动选定推荐项
