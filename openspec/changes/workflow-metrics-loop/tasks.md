...
- [ ] 1.1 在 `sdflow-init/assets/workflow/` 下新建 `lens-metric` 锚契约规范（字段/取值域/归属规则/`sev` 子格式定序/版本 `v1` + **enum 扩展治理:新镜升 v2 勿塞 broad**〔SR-E〕），沿用 fence-aware 行级纪律；**契约加 MUST「review 报告中示范锚语法 MUST 包 ``` fence 内」**〔SR-N 闭合自指残差〕 [workflow-metrics: 度量锚契约]
- [ ] 1.2 若 bundle 有 INDEX/托管块，登记新规范；确认 `sdflow-init update` 托管刷新覆盖此文件（权威源纪律）[spec-workflow: bundle 权威源]

## 2. 生产者落锚（P0）

- [ ] 2.1 `sdflow-code-review/SKILL.md`：Step3 裁决后每镜落 `lens-metric` 锚；现 `voice分桶` prose 行被 outside-voice 镜锚**吸收取代**；报告格式台账区同步；只引用契约字段不复制 [spec-workflow: 评审每镜落度量锚 / workflow-metrics: 度量锚契约]
- [ ] 2.2 `sdflow-spec-review/SKILL.md`：Step3 每镜（领域/对抗/接地/outside-voice/broad）落 `lens-metric` 锚；只引用契约不复制 [spec-workflow: 评审每镜落度量锚]
- [ ] 2.3 两 SKILL：独立贡献在 Step3 去重时导出（记每条命中镜集合→`独立`标量）；锚存在性自检扩一类（**缺字段 或 `layer/lens/runner/sev` 取值越域 均阻塞**〔SR-C〕；数值一致性是主 session 信任边界、非机械门〔SR-B〕）；spec-review 采纳/裁掉**在设计门拍板回写时最终化**〔SR-M〕；旁路声明（锚有无 MUST NOT 改 findings 判定）[workflow-metrics: 独立贡献导出 / spec-workflow: 旁路不改判定]
- [ ] 2.4 机械核对：grep 两 SKILL 全文无残留 `voice分桶` prose 指令、字段清单仅引用权威源 [spec-workflow: 取代 voice 分桶]

## 3. 只读聚合脚本 + 测试（P1，TG-18）

- [ ] 3.1 写 `workflow/tools/` 下只读聚合脚本：扫所有 `archive/**/*-review-report.md` 的 `lens-metric` 锚 → 多列可排序表；**字段提取解析器为净新路径**（现有 `_line_scoped_hits` 只做存在性检测），**在本脚本内重实现 ~15 行 fence 核**（`in_fence` 翻转 + 锚独占行前缀 `<!-- sdflow:lens-metric v1` + 受限 kv 提取，**禁裸 `split`/substring**），**MUST NOT 跨 skill import `ship_gate`**；无锚老报告显式计「无锚样本 N，不纳入」；MUST NOT 产合成分、MUST NOT 写新持久文件 [workflow-metrics: 只读聚合 view]
- [ ] 3.2 pytest 正例：≥2 归档锚聚合成表，各列取值正确 [workflow-metrics: 只读聚合 view]
- [ ] 3.3 pytest 反例矩阵（**镜像 ship_gate fence 用例**）：锚在未闭合/闭合 fenced 代码块内不误取（**含 design 文档式示例锚**）· 措辞漂移 · 字段缺失 · 取值越域 · **`sev` 子格式健壮（省级/乱序/多空格）**〔SR-I〕 · 裸 substring 陷阱均不腐坏解析；无合成分断言 [workflow-metrics: 只读聚合 view]
- [ ] 3.4 端到端断言：聚合表 `独立` 列非空（去重导出通路打通）[workflow-metrics: 独立贡献导出]
...
- [ ] 5.1 按代码实况核 `specs/workflow-metrics/spec.md` + `specs/spec-workflow/spec.md` delta 与六落点措辞一致；`openspec validate workflow-metrics-loop` 通过 [全需求]
- [ ] 5.2 开发 checkout 跑 `bash setup.sh`（改 `assets/workflow` 才让全局 canonical 生效、测得到）[spec-workflow: bundle 权威源]
