...
| [T14](open/todo/T14.md) | todo | PROPOSED | 2026-07-01 | `setup.sh` | Windows 指针分支补所有权检查（workflow-path 被异物占位时停手告警，同 Unix 分支） |
...
| [T16](open/todo/T16.md) | todo | PROPOSED | 2026-07-01 | `setup.sh` | install_sdflow 告警独立打印分支，不复用 skipped 数组（现输出中英文案叠加） |
| [T18](open/todo/T18.md) | todo | PROPOSED | 2026-07-01 | `setup.sh install_into` | skills 软链切换（install_into 对既有软链 ln -snf）无指向变更提示——与 canonical 接管可见化(impl-review-fix)对齐 |
...
| [T24](open/todo/T24.md) | todo | PROPOSED | 2026-07-01 | `setup.sh install_into 软链分支` | install_into 对既有软链零所有权校验——同名异物软链被 ln -snf 无声覆盖（已复现）；需专门设计「何为自属目标」再修，与 T18（可见性）分立 |
...
| [T86](open/todo/T86.md) | todo | PROPOSED | 2026-07-01 | `anchor_lint.py load_enums` | 未闭合 fence 不 fail-closed（与 emitter _read_block_pairs 同盲区，本 change 已修 emitter 侧）——EOF 前无闭合围栏时静默把剩余全文当块体；契约受版本控制利用面低，但两侧同错致等价性测试假绿风险 |
...
| [T150](open/todo/T150.md) | todo | PROPOSED | 2026-07-01 | `sdflow-init/assets/hack/outside-voice.sh` | preflight 只 command-v + timeout 检查,未按 ADR-6「真跑一次」补低成本真探针(CLI未认证/模型无效/参数不支持仍返回ready,失效漏到exec归exec-error) |
...
| [T228](open/todo/T228.md) | todo | PROPOSED | 2026-07-01 | sdflow-init/assets/hack/outside-voice.sh: secret_scan | 含 NUL 字节的输入让 secret_scan 漏判，且命中二进制文件时 stderr 打印 Binary file <path> matches 而非纯行号 |
