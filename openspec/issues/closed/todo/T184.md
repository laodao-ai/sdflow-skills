...
closed_reason: "(a) exec 自足化已由 T175 修法覆盖(内联环境变量前缀)；(b) helper 自己调 resolve-models.sh 被 ADR-9/GC-7 机械锁禁止(TestOutsideVoiceDoesNotSelfResolve)"
---
...
> 2026-08-04 状态：PROPOSED → WONTDO（(a) exec 自足化已由 T175 修法覆盖(内联环境变量前缀)；(b) helper 自己调 resolve-models.sh 被 ADR-9/GC-7 机械锁禁止(TestOutsideVoiceDoesNotSelfResolve)）
