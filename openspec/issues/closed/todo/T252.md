---
id: "T252"
pool: "todo"
status: "WONTDO"
priority: null
type: "基础设施"
date: "2026-07-01"
source_change: "harden-implement-review-loop"
module: "openspec/adr/0031-t10-label-split-by-decision-semantics.md"
summary: "adr/0031 承诺的「T10 单一源化留待独立立项」未落成任何 todolist 条目，而该类漂移已真实发生过一次"
resolved_by: "main"
closed_date: "2026-08-05"
closed_reason: "T10-choice 90处引用,单一源化超出清理批次;追踪条目本身就是这条issue;adr/0031已拆分具名规则"
---
## T252: adr/0031 承诺的「T10 单一源化留待独立立项」未落成任何 todolist 条目，而该类漂移已真实发生过一次
> adr/0031 承诺的「T10 单一源化留待独立立项」未落成任何 todolist 条目，而该类漂移已真实发生过一次

**关联文档**：`openspec/changes/harden-implement-review-loop/design.md`

**动机**：spec-review L6(Claude CEO)：ADR 写了「留待独立立项」但没有任何追踪条目，等于没留。且 C8 差异 B(spec-workflow spec 丢「按三镜+主次」)就是该架构导致的真实漂移。本条即补上该追踪。

**思路**：把 T10-choice 的 15 处复述重构为单一源 + 指针引用(仿 model-tiers.md 的机读块模式)，或至少补一条 lint 守住 15 处措辞一致。
> 2026-07 状态：OPEN → PROPOSED

> [迁移自批次 harden-implement-review-loop] 原计划: <待填>
> 2026-08-05 状态：PROPOSED → WONTDO（T10-choice 90处引用,单一源化超出清理批次;追踪条目本身就是这条issue;adr/0031已拆分具名规则）
