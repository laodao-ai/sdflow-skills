...
| 阶段 | 内容 | 涉及 issue | 就绪度 |
|---|---|---|---|
| **P1** · issues 读写路径加固 | 读取路径诚实化 + reindex 丢数据 bug | T231 + B12 | ✅ 已完成（change `harden-issues-read-write`，2026-08-02 归档） |
| **P2** · 安全与锚一致性 | voice 路径注入 + anchor_lint 镜名缺失 | T164 + T148 | ✅ 已完成（change `fix-voice-quoting-and-mirror-vocab`，2026-08-03 归档） |
| **P3** · gate 基础设施 | archive 打断测试 + tickets 存储改造 | T179 + T244 | ✅ T179 WONTDO（dogfood-only 低性价比）；T244 暂不排期 |
...
```
  P1 (issues 读写) ──独立──▶ ✅ 已完成（harden-issues-read-write）
  P2 (安全+锚)    ──独立──▶ ✅ 已完成（fix-voice-quoting-and-mirror-vocab）
  P3 (gate T179)  ──独立──▶ ✅ T179 WONTDO / T244 暂不排期
  P4 (下游推送)   ──独立──▶ ✅ 已完成
```
