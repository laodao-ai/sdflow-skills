...
- Leg 2（前置）P0 基线：有 per-阶段墙钟基线（checkpoint 时间戳收成）+ **明确收益门槛数值**，作为 P2/P3 立项与验收判据。**✅ 已收口**（`sdflow-retro` 全 18-change 聚合 `openspec/retro/report.md`，2026-07-06；分析结论见 design §2.2 收口段 + D11/D12）。

  **P0 定案的收益门槛（P2/P3 立项与验收总纲）**：

  | 维度 | 门槛 | 依据 |
  |---|---|---|
  | **P2 主指标 = token** | 机械镜（接地/历史）可查实跑 light 档 + **token/轮可测下降**（opus→light）。**⚠️ 诚实边界（2026-07-31 闭合）**：harness 不暴露 per-子代理 token 用量 ⇒ 「可测下降」降级为「resolver 输出正确（`SDFLOW_TIER_LIGHT=haiku`）+ 指令已落地（两审 SKILL.md 明写 `$SDFLOW_TIER_LIGHT`）= 充分条件」；实测降是理论必然（opus→haiku），但无可采集的 per-镜 token 数据来定量验证 | 18-change 实测 spec-review 43% 被人类门 elapsed 主导、机械镜并行且非最慢镜 → 墙钟撬不动、token 真省 |
  | **P2 墙钟** | 降级为**不回归**（不设下降门槛；单镜墙钟对基线不升即可） | 机械镜非聚合墙钟关键路径（D11） |
  | **安全（全 Leg）** | **误免率恒 0** + 降档/流水线/后台异常**可观测 fail-closed 退回强审**，不静默丢镜 | 元原则「评审覆盖不得无声蒸发」 |
  | **砍镜闸门（度量回路）** | per-(层,镜) **出现轮数≥10 ∧ 独立率<20% ∧ 采纳率<50%（连续 2 复评窗）** 才议降采样/淘汰；未达则一律保留 | 价值锚仅 3 change、轮数全<10、已测镜均高价值（接地镜独立率 75%）；阈值为保守起始值，积累后可校（D12） |
  | **墙钟真杠杆归属** | 聚合墙钟（43% spec-review 人类门）只有 **Leg3 降轮次**动得了；Leg2 每轮机械镜提速对聚合墙钟近乎不可见 | 战略权重上调 Leg3、下调 Leg2 墙钟预期（D11） |

- Leg 2：接地/历史镜跑快档（**置信过滤/判断镜不降档**）、接地镜可提前、fan-out 可后台——**主收益 = token 省 + 关键路径串行段缩短**（非聚合墙钟，见 P0 门槛）；单镜墙钟对 P0 基线**不回归**；快档失败/后台异常**可观测 fail-closed 退回强审**，不静默丢镜。
