# batch-triage Specification

## Purpose
规范 `openspec/issues/` 待处理项（buglist/todolist）在 consolidation-planning 阶段的分诊行为：
把每个待处理项归入且仅归入「相关合批」（BASE-18 AND 门）、「大扫除批」（无逻辑面 ∧ 低危 ∧
非行为面路径的正交项，走一 change 一轮评审、item 粒度一项一 commit）、「单开 change」三者之一，
并以 issue 级 pre-diff 纯规则判据（fail-closed、同类 Leg1 路径守卫、无脚本自动兜底）约束大扫除
批的准入与聚合上限。本能力当前落点为本仓-local（`openspec/issues/batch-triage-rules.md` +
`consolidation-plan.md`），发布给下游消费仓 deferred 至本仓 dogfood 验证之后。
