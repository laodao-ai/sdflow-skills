# devenv-provisioning Specification

## Purpose
`sdflow-devenv` 把项目的开发/测试环境真正建起来：为**任何项目**产出一份三层（单元/集成/e2e）测试与验证策略框架，一层都不许留白（留白允许，但 MUST 被问到、被看见）；落脚手架、尽可能真跑一遍确认；产出 `testing-strategy.md`（机械渲染）与 `environments.md`（逐槽问出来，归人 own）两份文档 + 入口索引。skill 是**副驾**——辅助搭建环境 + 提醒操作者别忘了考虑什么，不是替人开的生成器，也不是查人岗的审计官。机械层边界 = **防漏，不防伪**：只拦人看不见的（状态拼错 · 路径穿越 · 坏 JSON），人一眼看得见的（六槽留白）只报不拦。
...
状态迁移 SHALL 只能由**执行者本人**产出证据——机械层 SHALL 拒绝任何试图跳过 `verify-lane` / `confirm-lane` 直接把泳道写成 `verified` 的路径。
...
skill 执行验证命令时 SHALL 遵守以下「不伤害」边界，MUST NOT 偷跑或在诊断之外做修复：
...
机械载体 SHALL **只有一份 JSON**（`openspec/architecture/.devenv.json`，含 `layers` + `lanes`），且 MUST NOT 含 digest 或除 `layer` 外的封闭枚举。`environments.md` **完全在机械层之外**（见下）。
...
两份文档 SHALL 按「**测试 vs 非测试**」切线划分职责，MUST NOT 按「方法 vs 操作」切分：
...
skill SHALL 按以下固定五步顺序执行，MUST NOT 颠倒顺序：
...
本 skill 的触发范围 SHALL 限定为「建项目 dev/test 环境」，MUST NOT 与 `sdflow-init` / `sdflow-roadmap` / `sdflow-architecture` 的触发范围重叠：
