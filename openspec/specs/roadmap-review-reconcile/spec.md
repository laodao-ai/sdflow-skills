# roadmap-review-reconcile Specification
...
校验器 SHALL 断言 roadmap task-log 的 `## Review 处置` 小节存在且非空（非仅脚手架注释 / 空白），输出 reason_code `section-missing|section-empty|section-ok-DISPOSITION-UNCHECKED`〔grill-amendment Q2：输出码点明「逐条处置未核」，防 `section-ok-DISPOSITION-UNCHECKED` 被误读为已核=假绿，adr/0018〕。`section-ok-DISPOSITION-UNCHECKED` 退出码 0。小节缺失 MUST NOT 被当作「无未处置条目」真空通过。
...
- **THEN** 输出 `section-ok-DISPOSITION-UNCHECKED`，退出码 0
...
- **THEN** 校验器判 `section-ok-DISPOSITION-UNCHECKED`，MUST NOT 因子串「未处置」误报为存在未处置条目

#### Scenario: 不冒充逐条完整性
- **WHEN** 小节存在且非空但某条目实际缺状态标记
- **THEN** 校验器仍只输出 `section-ok-DISPOSITION-UNCHECKED`（存在 + 非空达成），逐条完整性交模型判断——脚本不假装覆盖
