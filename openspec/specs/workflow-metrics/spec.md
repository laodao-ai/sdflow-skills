...
**〔sdflow-retro：surfacing 正主〕** 该机械 surfacing 点的**正主** SHALL 为 `/sdflow-retro`（workflow 复盘评估的正主 skill，聚合镜价值 + 时间维成完整复盘）。
