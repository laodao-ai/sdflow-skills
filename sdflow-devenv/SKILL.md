...
## 🟢 两条问话纪律（副驾怎么问 —— 与红线对称：红线是 MUST NOT，这两条是 MUST DO）

> **人的注意力是这个 skill 最贵的资源，而且是唯一一种你消耗掉就补不回来的资源。**
> 十五个问题里，每一个「你们前端用什么测试框架？」，都在挤掉一个「**你上次被这个环境坑到，是什么事？**」

### P1 · 能查的自己查，能自己调研的自己调研

**问出口之前先过一遍**：这个答案，**在仓库里 / 在这台机器上 / 在公开资料里**吗？在 ⇒ **自己去拿**，拿到了**直接给结论**，不要拿它去占用人的注意力。
...
| 「这个项目有几个 Go 包？」 | `go list ./...` |

**查完了给结论，不给过程**：「你们的集成测试是 `make integration`（真 mosquitto + TLS，我跑过了，绿）」——**而不是**「我看到 Makefile 里有个 integration target，你确认一下是不是它？」

### P2 · 不确定的方案，先调研再给推荐 —— **MUST NOT 甩开放题**

拿不准的时候，正确动作**不是把选项原样丢给人**，而是**先把能查的都查了，带着推荐和依据进人门**。
**人做的是拍板，不是替你做调研。**

> ❌ **甩开放题**（这是试点里真实发生的）：
> 「⚠️ 待定：Windows 包到底怎么产出？（买/借一台 Windows 机器？上 GitHub Actions 的 windows runner？还是把 Windows 降级为 non-goal？）」
> ——三个选项，**零调研、零推荐**。人接到的是一道作业题。
>
> ✅ **调研后给推荐**：
> 「**Windows 包建议走 GitHub Actions 的 windows runner。**
> 依据：① 本仓已有 `.github/workflows/`，加一个 job 即可 ② wails 官方支持在 windows runner 上原生打包 ③ 公开仓 runner 免费。
> **代价**：签名需要证书，第一版只能出未签名包（用户首次运行会看到 SmartScreen 警告）。
> **备选**：降级为 non-goal —— 后果是 Windows 用户完全没有可用产物。
> **要不要这么定？**」

---

### 三分判据（P1/P2 的落地口径 —— 每一个问句先归一次类）

| 答案在哪 | 你怎么做 |
|---|---|
| **在仓 / 在机器 / 在公开资料里** | **自己查** → 给结论。**不问。** |
| **不在，但查得到候选和依据**（选型 / 路线 / 工具） | **调研 → 给推荐 + 依据 + 代价 + 备选** → **让人拍板**（P2） |
| **只在人脑子里**（偏好 · 踩过的坑 · 拍板权 · 组织约束） | **问**——**而这才是你该把注意力花光的地方** |

> **第三类问句的样子**：「你自己最近一次被这个环境坑到，是什么事？」「发出去炸了怎么退回来？」「Windows 到底要不要？」
> **它们没有标准答案，你查不到，只能问。⑥ 槽和「常见坑 / 回滚」两槽，几乎全属此类。**
