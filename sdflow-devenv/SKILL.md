...
- **`domains` 分域写入判据**〔scope-aggregate-suite-by-diff-domain · adr/0045〕——`domains` 声明的是**已存在的构建级隔离**，不是发明分区：
  - **判据（任一成立即算构建级隔离）**：该分区有独立 build tag / 独立 module（或等价的包级隔离）/
    项目运行期代码不 import 该分区路径。三者满足其一 ⇒ 推荐写入 `test-suites.domains`——分区路径归一个非默认域
    的 `paths`、该分区的跑法写进该域 `layers`；其余路径归一个 `default: true` 的域，其 `layers` 承接
    今日的 `unit`/`integration`/`e2e`。**三者均不成立 ⇒ MUST NOT 写 `domains`**（`test-suites` 只保留逐层命令，
    不引入分域维）。
  - **核验尽量让工具自己回答，MUST NOT 解析构建文件**：如 Go 项目用 `go list -deps <运行期包...>` 反查
    结果是否包含该分区路径（不含 ⇒ 隔离成立的信号之一）；其余栈类推（用该栈的官方依赖查询原语，不手写
    构建文件解析器）。**无法核验时 SHALL 向人说明并默认不写**，MUST NOT 猜。
  - **尾斜杠约定**（与 `impl_route.py suite-scope` 的匹配语义一致：以 `/` 结尾 = 目录前缀匹配，
    不以 `/` 结尾 = 精确路径相等）：分区目录一律以 `/` 结尾写入非默认域 `paths`；**层命令自身引用的文件**
    （如测试运行脚本 `hack/run-tests.sh`）SHALL 以**精确路径（不带尾斜杠）** 写入**默认域** `paths`——
    该精确路径比目录前缀更长，最长前缀胜出，使改动运行脚本本身的 change 归默认域而非工具域。
  - **已有 `domains` 配置时 MUST NOT 覆盖**——与上面「已有配置不覆盖」同纪律，不做二次调优。
