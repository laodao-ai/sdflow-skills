...
| **决策** | 对比表 | 多方案 side-by-side（候选 × 评估维度，含三镜后果） |
| **规划** | 阻塞 DAG | ticket 节点 + Blocked-by 有向边（关键路径可视） |
...
| design 含 ≥2 候选方案 | **对比表**（候选 × 评估维度，含三镜后果） |
| proposal Impact 涉及 ≥3 模块 | **模块关系图**（受影响模块 + 依赖方向） |
| tasks Blocked-by ≥3 条边 | **阻塞 DAG**（ticket 节点 + 有向边，标关键路径） |
...
| 阶段 | 画什么 |
|------|--------|
| **proposal.md** | 影响 ≥3 个模块时，Impact 节**鼓励**加模块关系图（谁受影响、依赖方向） |
| **design.md** | 结构图（默认）+ 命中触发的行为图（序列 / 状态机 / 数据流 / 决策）+ 多方案决策时**鼓励**加对比表（side-by-side 候选 × 评估维度） |
| **tasks.md** | Blocked-by 关系 ≥3 条边时**鼓励**加 ASCII DAG 图（ticket 节点 + 阻塞边）；**测试覆盖图**（此时测试策略才具体，比 design 期更自然） |
...
- [ ] design 含 ≥2 候选方案时，是否有对比表（候选 × 评估维度）？
- [ ] proposal Impact 涉及 ≥3 模块时，是否有模块关系图？
- [ ] tasks Blocked-by ≥3 条边时，是否有阻塞 DAG？
- [ ] 测试覆盖图是否在 tasks 阶段产出？
