...
若项目已维护产品级 PRD（放在哪由项目自定，本工作流不规定位置），功能级变更这样接：
...
新功能 idea
      ↓
/sdflow-spec（相位 A 澄清 → 相位 B 拷问 → 相位 C 生成四件套）
  ──── proposal.md 的 Why 节引用 PRD 对应目标 ────
  ──── 必填：成功指标（阻塞级字段） ────
      ↓
/sdflow-spec-review（设计审多镜，业务方向与技术细节同轮过）
      ↓
设计 HARD-GATE：人拍板
      ↓
specs/*.md 冻结，进阶段三
...
- 关键路径耗时：当前 N/A → 目标 < 30 秒 — 用 E2E 测试计时
- 操作失败率：当前 ~15% → 目标 < 5% — 从错误日志统计
