...
**与 sdflow-code-review 的裁决动作层是有意的不对称**（与"代码即 ground truth 故 sdflow-code-review 去接地镜"同类）：两者共享同一套**机械引用核**前置（`findings_ref_check.py`，DD4/adr-0041——只裁「引用不实」这一类确定性信号，pass/fail/uncheckable 三态），但通过机械前置之后、遇到**纯判断分歧**（引用属实、但是否成立见仁见智）时的去向不同：
...
| 对纯判断分歧项 | **强档二元裁决：采纳/裁掉/defer**（不问人） | **标注 + 上抛决策登记区，人工设计门拍板，不丢** |

一个「这边界场景 spec 没覆盖」类判断分歧，在代码侧可由强档二元裁决当场判"不成立"裁掉（阶段三无人类门），在设计侧恰是**最该 surface 给人的高价值捕获**——故 sdflow-spec-review 的判断分歧项**不下放给自动裁决**。它的分流是第三步**对抗裁决**（强模型、带上下文）+ 第四步**决策登记区**上抛人工设计门，而非任一方自动丢弃。
