...
def _env(fake_claude, extra=None, run_dir=None):
    ...
    env["SDFLOW_CLI_PROBE_TIMEOUT"] = "60"   # B28: 全量跑负载下 5s 产品默认易假红；此夹具专属放宽
...
def test_preflight_ready_on_supported_version_and_agents_json(job_home, fake_claude):
    ...
    assert payload["reason_code"] == "ready", payload.get("checks")
...
def test_preflight_fails_closed_on_old_claude_version(job_home, fake_claude):
    ...
    assert payload["reason_code"] == "preflight-error", payload.get("checks")
...
def test_preflight_non_posix_skips_cli_probes_without_claiming_path_missing(monkeypatch, job_home):
    ...
    assert result["ok"] is False
    assert result["reason_code"] == "preflight-error", result.get("checks")
...
def test_dispatch_returns_within_monotonic_deadline_with_verified_job_id(job_home, fake_claude, repo):
    ...
    assert payload["reason_code"] == "ok", payload.get("checks")
...
def test_dispatch_reclaims_process_tree_and_reserve_when_deadline_expires(job_home, fake_claude, repo, tmp_path):
    ...
    payload = _json_stdout(proc)
    assert proc.returncode == 1
    assert payload["reason_code"] == "exec-error", payload.get("checks")
...
def test_reserve_without_job_metadata_is_reserved_and_never_collectable(tmp_path):
    ...
    assert payload["reason_code"] is None, payload.get("checks")
...
def test_installed_snapshot_passes_preflight_from_the_global_home(tmp_path, fake_claude):
    ...
    assert proc.returncode == 0, proc.stdout + proc.stderr
    payload = _json_stdout(proc)
    assert payload["ok"] is True and payload["reason_code"] == "ready", payload.get("checks")
    assert payload["checks"]["capability-manifest"]["ok"] is True
...
def test_installed_snapshot_fails_closed_on_manifest_skew_with_a_refresh_hint(
    ...
    assert proc.returncode == 1
    payload = _json_stdout(proc)
    assert payload["ok"] is False and payload["reason_code"] == "preflight-error", payload.get("checks")
    assert payload["checks"]["capability-manifest"]["ok"] is False
