...
# v1 的 test_windows_local_disk_acquire_conflict_replace_cleanup（recorder_lock 获取/冲突/
# 释放端到端 + participant/delegation-chain 校验）已随 issues-v2-single-file-model Task 3
# 删除：v2 单文件模型整体移除了仓级 `recorder_lock` 协议（issues_v2.py 无 recorder_lock /
# recorder_child_env / validate_recorder_participant），场景前提在 v2 architecture 下不再
# 成立，不做迁移。sdflow-init 的 `merge_runtime_gitignore` 冒烟覆盖与 recorder_lock 无关，
# 保留在 sdflow-init/tests/test_runtime_gitignore.py（超出本 change 范围，未搬移）。
...
def test_windows_setup_uses_owned_copies_and_refreshes_them(tmp_path):
    ...
    for host in (".claude", ".codex"):
        ...
        assert (installed / "scripts/issues_v2.py").read_bytes() == (
            ROOT / "sdflow-issues/scripts/issues_v2.py"
        ).read_bytes()


# ── repo_root 的 Windows 泳道覆盖（change harden-repo-root-fail-closed · tasks 4.6 / CF-6） ──
#
# 为什么这些用例落在**本文件**而不是 test_repo_root_identity_issues.py：
...
# realpath 对 SUBST）此前**从未在 Windows 上真跑过**。（issues-v2-single-file-model · Task 3
# 改造：原两个用例直传 tmp_path 给 v1 recorder_lock，绕开 repo_root——该机制已在 v2 移除，
# 对应用例随之删除；下方 repo_root 用例改指向单文件模型的 `issues_v2.py`。）
...
def _repo_root_fn():
    return _load("issues_v2_win_repo_root", "sdflow-issues/scripts/issues_v2.py").repo_root
