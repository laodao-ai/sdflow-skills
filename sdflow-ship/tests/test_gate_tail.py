from conftest import commit_all, mkchange, head_sha, write_report, write_anchored_report
...
def test_cr_blocked_exit4(repo):
    d = impl_done(repo)
    # [mlh-p5 Task5] live 迁 frontmatter〔sweep-pool-debt D3/D4〕锚 = code 域内容指纹
    write_anchored_report(repo, d, "code-review-report.md", head_sha(repo), "code",
                          body="# 代码审报告\n", code_review="blocked")
    commit_all(repo, "cr")
...
def test_verify_fail_exit5(repo):
    ...
    write_anchored_report(repo, d, "code-review-report.md", head_sha(repo), "code",
                          body="# 代码审报告\n", code_review="pass")
    write_anchored_report(repo, d, "verify-report.md", head_sha(repo), "code",
                          body="# 验证报告\n", verify="FAIL")
