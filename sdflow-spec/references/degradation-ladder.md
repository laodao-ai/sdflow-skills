...
- **判据是「错误类别」，不是「次数」**：瞬时错误重试一次；契约错误不重试〔A-1〕。
...
> 正文只放最终态；本附录承载「为什么是这样」（`openspec/rules/doc-authoring.md` DOC-1）。

**〔A-1〕重试次数为什么恰好是 1** —— 无强依据。按通则④判为可接受边角：
换成 2 次或 3 次都不改变失败面的形状，而**错误类别的二分**（瞬时 / 契约）才是承重的那条。
**MUST NOT 为「到底几次」单独返工。**
