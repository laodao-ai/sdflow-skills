...
> **本 skill 仅服务本仓自身**：两子命令起手都会校验 cwd 位于 sdflow 仓
> （git remote 含 `laodao-ai/sdflow`，即脚本常量 `EXPECTED_REMOTE_SUBSTR`），非本仓
> fail-loud 退出、不写任何文件（proposal A4）。在其他项目里被误触发时，直接如实告知
> 用户「本 skill 仅服务 sdflow 工具链自身，此仓不适用」，不要尝试变通执行。
