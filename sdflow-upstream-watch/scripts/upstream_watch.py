...
本脚本 MUST 从 sdflow 仓内某处运行（cwd 守卫据此判定，不限定必须是仓根——
...
EXPECTED_REMOTE_SUBSTR = "laodao-ai/sdflow"
...
class CwdGuardError(Exception):
    """cwd 非 sdflow 仓（proposal A4）——fail-loud，调用方 MUST NOT 写任何文件。"""
...
def guard_cwd():
    """cwd 守卫（proposal A4）：确认调用者的 cwd 位于 sdflow 仓（git remote 判定），
...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        description="四源上游追踪（gstack/superpowers/matt/OpenSpec）——collect 采集事实，"
                     "advance 校验报告后推进锚。仅服务 sdflow 仓自身。",
    )
